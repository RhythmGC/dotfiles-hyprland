...
Singleton {
    ...
    readonly property string previewFile: "/tmp/ba-gowall-preview"
    ...
    readonly property string _shimDir: "/tmp/ba-gowall-shim"
    ...
    function convertCurrentTheme(sourcePath: string, format: string): void {
        ...
        if (palette.length === 0) { error = "Current BlueArchive theme palette is unavailable"; return }
        const json = JSON.stringify({ name: "ba-current", colors: palette }, null, 2)
        const fmt = _ext(format)
        _runOperation(["/usr/bin/gowall", "convert", src, "--theme", _customThemePath, "--output", _previewPathFor(fmt), "--format", fmt], fmt, json, true, _basenameWithoutExt(src))
    }
    ...
    Process {
        ...
        onExited: (exitCode) => {
            ...
            const isWaffle = (Config.options?.panelFamily ?? "ba") === "waffle"
            ...
            if (waffleOwnWallpaper) {
                ...
                // ba, or waffle following main → full pipeline on main wallpaper
