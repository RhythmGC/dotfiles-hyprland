...
Singleton {
    ...
    Process {
        ...
        command: [
            ...
            + "ba_managed=no; "
            + "grep -q '^# ba-autostart' \"$f\" 2>/dev/null && ba_managed=yes; "
            + "echo \"$name|$enabled|$kind|$desc|$wanted|$ba_managed\"; "
            + "done"
        ]
        stdout: SplitParser {
            onRead: (line) => {
    ...
    Process {
        ...
        function remove(name) {
            ...
            const cmd = "/usr/bin/systemctl --user disable --now '" + name
                + "' 2>/dev/null || true; "
                // Only remove units that were created by ba Autostart (marker comment)
                + "if grep -q '^# ba-autostart' '" + dir + "/" + name + "' 2>/dev/null; then "
