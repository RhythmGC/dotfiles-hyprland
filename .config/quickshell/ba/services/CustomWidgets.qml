...
Singleton {
    ...
    readonly property string widgetsDir: `${Directories.configPath}/ba/widgets`
    ...
    IpcHandler {
        ...
        function create(name: string): string {
            if (!name || name.length === 0) return "Usage: ba customWidgets create <name>";
            root.create(name);
            return `Creating widget "${name}" in ${root.widgetsDir}/${name}/...`;
        }

        function remove(widgetId: string): string {
            if (!widgetId || widgetId.length === 0) return "Usage: ba customWidgets remove <id>";
...
// ${_createProcess._pascalName} — custom BlueArchive desktop widget
