 ...
 * - Cache in ~/.cache/ba/window-previews/
...
Singleton {
    ...
    readonly property string previewDir: FileUtils.trimFileProtocol(Directories.genericCache) + "/ba/window-previews"
    ...
    function _saveClipboard(): void {
        const tmpFile = "/tmp/ba-clipboard-qml-" + Date.now() + ".tmp"
