...
Singleton {
    ...
    function restart(): void {
        ...
        Qt.callLater(() => {
            Quickshell.execDetached(["systemctl", "--user", "restart", "ba.service"])
        })
    }
    ...
    function _notifyUser(): void {
        ...
        Notifications.send(
            "BlueArchive",
            Translation.tr("Memory usage is high (~%1 MB accumulated). A restart would free it. Run: ba memory restart").arg(mbEstimate),
