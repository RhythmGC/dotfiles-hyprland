...
Singleton {
    ...
    readonly property bool _debugWallpaperUrls: (Quickshell.env("BA_DEBUG_WALLPAPER_URLS") ?? "") === "1"
    ...
    readonly property bool isWaffleFamily: (Config.options?.panelFamily ?? "ba") === "waffle"
    ...
    function currentThemingWallpaperPath(monitorName = ""): string {
        ...
        if (root.useBackdropWallpaper || root.useBackdropForColors) {
            ...
            const baBackdrop = Config.options?.background?.backdrop ?? {}
            if (baBackdrop.useMainWallpaper ?? true)
                return mainPath
            ...
            return baBackdrop.wallpaperPath || mainPath
        }
    ...
    function currentWallpaperPathForTarget(target = "main", monitorName = ""): string {
        ...
        case "backdrop": {
            const baBackdrop = Config.options?.background?.backdrop ?? {}
            const useMainWallpaper = baBackdrop.useMainWallpaper ?? true
            if (useMainWallpaper)
                return mainPath
    ...
    function applySelectionTarget(path, target = "main", darkMode = Appearance.m3colors.darkmode, monitorName = "") {
        ...
        case "backdrop":
            ...
            return
        case "waffle":
            if ((Config.options?.panelFamily ?? "ba") === "waffle")
    ...
    Process {
        ...
        environment: ({
            "BA_VENV": Quickshell.env("BA_VENV") || Quickshell.env("HOME") + "/.local/state/quickshell/.venv",
            "BA_VENV_COMPAT": Quickshell.env("BA_VENV") || Quickshell.env("HOME") + "/.local/state/quickshell/.venv"
        })
        stdout: SplitParser {
            onRead: data => {
