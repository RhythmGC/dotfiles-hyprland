...
/*
 * BlueArchive shell update checker service.
 ...
 *
 * NOTE: The config directory (~/.config/quickshell/ba) is NOT a git repo.
...
Singleton {
    ...
    readonly property string manifestPath: configDir + "/.ba-manifest"
    ...
    readonly property string unavailableMessage: managedExternally
        ? "This BlueArchive installation is managed outside the runtime copy. Use your package manager or installation workflow to update it."
        : "Repository not found. The update system cannot locate the BlueArchive git repository."
    ...
    onHasUpdateChanged: {
        ...
        Notifications.notify({
            summary: "BlueArchive Update Available" + version,
    ...
    function openOverlay(): void {
        ...
        if (!panels.includes("baShellUpdate")) {
            Config.setNestedValue("enabledPanels", [...panels, "baShellUpdate"])
        }
    ...
    function performUpdate(): void {
        ...
        const termTail =
            "echo; " +
            "if [ $rc -eq 0 ]; then " +
                "echo 'All good — BlueArchive updated successfully. The shell will restart on its own.'; " +
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // `setup update` triggers `systemctl --user restart ba.service` as its
    ...
    Process {
        ...
        command: [
            "/usr/bin/bash", "-c",
            "p='" + root.configDir + "'; [[ ( -e \"$p/.git\" || -e \"$p/../../../.git\" ) && -f \"$p/setup\" && -f \"$p/shell.qml\" ]] && echo OK || echo ''"
        ]
        stdout: StdioCollector {
            onStreamFinished: {
    ...
    Process {
        ...
        command: [
            "/usr/bin/bash", "-c",
            "p='" + root.repoPath + "'; [[ ( -e \"$p/.git\" || -e \"$p/../../../.git\" ) && -f \"$p/setup\" && -f \"$p/shell.qml\" ]] && echo OK || echo ''"
        ]
        stdout: StdioCollector {
            onStreamFinished: {
                const ok = ((text ?? "").trim() === "OK")
    ...
    Process {
        ...
        command: [
            ...
            "for dir in ~/baOS ~/ba ~/BlueArchive " +
            "~/.local/src/baOS ~/.local/src/ba " +
            "~/Projects/baOS ~/Projects/ba " +
            "~/Downloads/baOS ~/Downloads/ba " +
            "~/src/baOS ~/src/ba; do " +
            "if [[ -d \"$dir/.git\" && -f \"$dir/setup\" && -f \"$dir/shell.qml\" ]]; then echo \"$dir\"; exit 0; fi; done; " +
            // Last resort: find in home (max depth 3, timeout 2s)
            "timeout 2 find \"$HOME\" -maxdepth 3 -name setup \\( -path '*/ba/setup' -o -path '*/baOS/setup' -o -path '*/ba/setup' \\) 2>/dev/null | while read -r f; do [[ -f \"$(dirname \"$f\")/shell.qml\" ]] && dirname \"$f\" && break; done; "
        ]
        stdout: StdioCollector {
            onStreamFinished: {
        ...
        onExited: (exitCode, exitStatus) => {
            if (exitCode !== 0) {
                ...
                if (root.consecutiveFetchErrors >= 3 && !root.fetchErrorNotificationShown) {
                    ...
                    const title = "BlueArchive Update Check Failed"
    ...
    Process {
        ...
        command: [
            ...
            "  elif [[ ( -e \"$repo/.git\" || -e \"$repo/../../../.git\" ) ]]; then " +
