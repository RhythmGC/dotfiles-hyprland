 ...
 * - Invoked via IPC: `ba globalActions run <actionId> [args]`
 * - Extended by users via scripts in ~/.config/baOS/actions/
 *
 * Architecture:
...
Singleton {
    ...
    function runLauncher(args): void {
        Quickshell.execDetached([Quickshell.shellPath("scripts/ba")].concat(args ?? []))
    }
    ...
    readonly property var _appearanceActions: [
        ...
        {
            id: "style-ba",
            name: Translation.tr("Style: BlueArchive"),
            description: Translation.tr("Switch to BlueArchive style"),
            icon: "terminal",
            category: "appearance",
            keywords: ["style", "ba", "theme"],
            execute: () => { Config.setNestedValue("appearance.globalStyle", "ba") }
        },
        {
            id: "style-angel",
    ...
    readonly property var _settingsActions: [
        ...
        {
            id: "switch-family-ba",
            name: Translation.tr("Switch to ba Panel Family"),
            description: Translation.tr("Use the Material ba panel layout"),
            icon: "dashboard",
            category: "settings",
            keywords: ["family", "panel", "ba", "material", "layout"],
            execute: () => {
                root.runLauncher(["panelFamily", "set", "ba"])
            }
        },
        {
