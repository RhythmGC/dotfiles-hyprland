...
> **Note on quickshell package:** BlueArchive intentionally uses `quickshell` from official repos to avoid long AUR compile times and update-time build failures.
>
...
```bash
git clone https://github.com/RhythmGC/dotfiles-hyprland.git ~/.config/quickshell/ba
```
...
This gives you:
- Niri config wired to the `ba` launcher
- Theming templates for Material You colors
- GTK settings
- Fuzzel config

### 4. Enable the BlueArchive user service
...
- `Mod+Space` (`Super+Space`) toggles the ba overview
- `Alt+Tab` cycles windows using ba's switcher
