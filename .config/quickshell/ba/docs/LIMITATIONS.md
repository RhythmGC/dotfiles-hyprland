...
ba is built for **Niri**. Some features were inherited from the original Hyprland version and don't work on Niri.
...
- **PipeWire required**: ba uses PipeWire APIs directly. PulseAudio-only setups won't work.
...
- **No live previews**: Unlike some shells, ba doesn't capture live window thumbnails. You see app icons, not actual window content.
- **Workspace snapshots disabled**: The code for workspace screenshots exists but is disabled (too slow/unreliable).

### Window Matching

- **App ID matching**: ba matches windows by `app_id`. Some apps (especially Electron apps) have weird or missing app IDs, causing icon mismatches.
- **Quickshell windows**: All ba windows have `app_id: "org.quickshell"`. This is correct, not a bug.

### Backdrop & Wallpaper

- **Separate configs**: Material ba and Waffle have independent backdrop/wallpaper settings. If you enable both families, each manages its own background layer.
- **Niri layer rules required**: The backdrop uses Niri's `place-within-backdrop` layer rule. If your wallpaper doesn't show in overview, check that your `config.kdl` has the layer rules for `quickshell:baBackdrop` and `quickshell:wBackdrop`.
...
- `lock.useHyprlock = true` in config makes the lock keybind launch Hyprlock instead of ba's lock screen. Only works on Hyprland.
...
- **Generated translations**: AI-generated translations go to `~/.config/baOS/translations/`. Quality varies.
...
- ba loads modules lazily, but a fully-loaded shell with all features enabled uses ~200-400MB RAM. Disable panels you don't use in Settings → General → Enabled Panels.
...
- ba spawns UI elements per-screen. Most features work, but some edge cases (like dragging windows between monitors in overview) aren't implemented.
...
- ba is Wayland-only. No X11 support, no XWayland workarounds for the shell itself. (Your apps can still use XWayland via `xwayland-satellite`.)

### Config Hot-Reload

- Most config changes apply immediately. Some (like enabling/disabling modules) require restarting BlueArchive with `ba restart`.
...
1. Check `ba logs` for errors - the answer is usually right there
2. Verify the feature isn't listed as a known limitation above - yes, you have to actually read this page
3. Test with a fresh config: `mv ~/.config/baOS/config.json ~/.config/baOS/config.json.bak`
