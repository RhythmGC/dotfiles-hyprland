...
BlueArchive exposes IPC targets you can call from Niri keybinds, scripts, or your terminal.

> **Quick discovery:** `ba help` lists all targets, `ba <target> --help` shows available functions.
> Shell completions: `eval "$(ba completions bash)"` (also zsh, fish).
...
```kdl
bind "Key" { spawn "ba" "<target>" "<function>"; }
```

For low-level debugging, `ba ipc <target> <function>` still works.

---

## Available Targets

Everything BlueArchive can do, exposed for your scripting pleasure.
...
```kdl
bind "Mod+Space" { spawn "ba" "overview" "toggle"; }
```
...
```kdl
bind "Super+G" { spawn "ba" "overlay" "toggle"; }
```
...
```kdl
bind "Super+Shift+I" { spawn "ba" "idle" "toggle"; }
```
...
```kdl
bind "Super+V" { spawn "ba" "clipboard" "toggle"; }
```
...
```kdl
bind "Alt+Tab" { spawn "ba" "altSwitcher" "next"; }
bind "Alt+Shift+Tab" { spawn "ba" "altSwitcher" "previous"; }
```
...
```kdl
bind "Super+Shift+S" { spawn "ba" "region" "screenshot"; }
bind "Super+Shift+X" { spawn "ba" "region" "ocr"; }
bind "Super+Shift+A" { spawn "ba" "region" "search"; }
```
...
```kdl
bind "Super+Shift+V" { spawn "ba" "voiceSearch" "toggle"; }
```
...
```kdl
bind "Super+Shift+E" { spawn "ba" "session" "toggle"; }
```
...
```kdl
bind "Super+Alt+L" allow-when-locked=true { spawn "ba" "lock" "activate"; }
```
...
```kdl
bind "Super+Slash" { spawn "ba" "cheatsheet" "toggle"; }
```
...
```kdl
bind "Mod+Q" repeat=false { spawn "ba" "close-window"; }
```
...
```kdl
bind "Super+Comma" { spawn "ba" "settings"; }
```
...
Navigate the settings overlay to a specific page (same as clicking the nav rail). Opening the window itself is the `ba settings` CLI command (target `settings` above).
...
```kdl
bind "Super+Slash" { spawn "ba" "globalActions" "open"; }
bind "Super+M" { spawn "ba" "globalActions" "run" "toggle-mute"; }
```
...
```kdl
bind "Ctrl+Alt+T" { spawn "ba" "wallpaperSelector" "toggle"; }
```
...
```kdl
bind "Ctrl+Alt+T" { spawn "ba" "coverflowSelector" "toggle"; }
```
...
```kdl
bind "Ctrl+Mod+Space" { spawn "ba" "mpris" "playPause"; }
bind "Mod+Alt+N" { spawn "ba" "mpris" "next"; }
bind "Mod+Alt+P" { spawn "ba" "mpris" "previous"; }
```
...
```kdl
bind "Mod+M+Space" { spawn "ba" "ytmusic" "playPause"; }
```
...
```kdl
bind "Super+F12" { spawn "ba" "gamemode" "toggle"; }
```
...
Switch between panel styles. ba supports two visual styles: Material ba (default) and Waffle (Windows 11-like).

| Function | Description |
|----------|-------------|
| `cycle` | Cycle to next panel family (ba → waffle → ba) |
| `set` | Set specific family ("ba" or "waffle") |

```kdl
bind "Mod+Shift+W" { spawn "ba" "panelFamily" "cycle"; }
```
...
```kdl
bind "Mod+Alt+K" { spawn "ba" "keyboard" "switchLayout"; }
```
...
Waffle Alt+Tab window switcher. Separate from the ba `altSwitcher` — supports quick-switch (first tab switches instantly, second opens UI) and no-visual-UI mode.
...
```kdl
bind "Super+W" { spawn "ba" "background" "toggleEditMode"; }
```
...
Custom widget management. Create, list, reload, and remove user-installed widgets from `~/.config/baOS/widgets/`.
...
These are top-level `ba` commands that work directly, without going through IPC.
...
```kdl
bind "Super+Shift+C" { spawn "ba" "colorpicker"; }
```
