...
| Switch panel family | Swap between ba and waffle |
...
Example: create `~/.config/baOS/actions/deploy.sh`:
...
```bash
ba globalActions list              # list all available actions
ba globalActions search "screen"   # search actions by name
ba globalActions run <id>          # execute an action by ID
ba globalActions runWithArgs <id> <args>  # execute with extra arguments
```
