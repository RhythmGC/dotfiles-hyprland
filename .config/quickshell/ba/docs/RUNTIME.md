...
Compositor reaches graphical-session.target
  |
systemd starts ba.service (wants link from compositor)
  |
ExecStart calls: /usr/bin/ba run --session
  |
ba script (bash):
...
Config.ready fires:
  ...
  |
Panel loading:
  - Selects ShellBaPanels or ShellWafflePanels based on panelFamily
...
The systemd service is the key piece. It does not use `systemctl enable` in the traditional sense because there's no `[Install]` section. Instead, BlueArchive creates a wants link from your compositor's service:

**Niri**: `~/.config/systemd/user/niri.service.wants/ba.service`

**Hyprland**: `~/.config/systemd/user/wayland-wm@Hyprland.service.wants/ba.service`

This means BlueArchive starts when your compositor starts and stops when it stops. It will never accidentally start under KDE or GNOME.

Managing the link:

```bash
ba service enable    # create the wants link
ba service disable   # remove it
ba service status    # check current state
```

## The ba launcher

`scripts/ba` is a 2400+ line bash script that wraps Quickshell. It's not the same as running `qs -c ba` directly:
...
For development and debugging, `qs -c ba` (direct mode) is usually better because you get stdout immediately. For daily use, the systemd service handles everything.
...
1. FileView reads `~/.config/baOS/config.json`
...
- If BlueArchive crashes, systemd restarts it (up to 3 times in 30 seconds)
- `ExecStopPost` runs `ba cleanup-orphans` to clear stale Quickshell runtime entries
...
Check `ba logs` for recent journal output, or `ba doctor` for automated diagnostics.
