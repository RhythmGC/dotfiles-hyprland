...
These are the default keybinds shipped with BlueArchive. They live in `~/.config/niri/config.d/70-binds.kdl` after install.
...
| Key | Action |
|-----|--------|
| `Mod+Space` | BlueArchive overview / app launcher |
...
| `Mod+Shift+W` | Cycle panel family (ba ↔ waffle) |
...
All media/volume/brightness keys are routed through BlueArchive IPC for OSD feedback.
...
See [IPC.md](IPC.md) for all available BlueArchive targets you can bind.

```kdl
// In 90-user-extra.kdl
binds {
    Super+P { spawn "ba" "session" "toggle"; }
}
```
