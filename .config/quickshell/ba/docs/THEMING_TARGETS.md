...
This document explains the first modular layer of the BlueArchive theming system.
...
```bash
ba theme apply terminals
```

Apply several:

```bash
ba theme apply gtk-kde editors chrome
```
...
```json
{
  ...
  "category": "custom",
  "inputs": ["palette.json"],
  "description": "Apply generated BlueArchive palette to My App.",
...
```bash
ba theme apply myapp
```

Inspect the generated target:

```bash
ba theme inspect myapp
```

Doctor-check it:

```bash
ba theme doctor myapp
```
