...
This document explains how BlueArchive is composed, how its main parts relate to each other, what each part is responsible for, and what changes usually affect when you touch it.
...
BlueArchive is a desktop shell built on Quickshell and QML.
...
This file defines the runtime composition for the `ba` family.
...
This file is the authoritative list of `ba` panels that the shell can materialize.

A change here affects module availability, startup cost, and the visible shell surface of the `ba` family.
...
It is not just a skin over `ba`. It is its own family with its own UI composition and visual identity.

That means a change that makes sense for `ba` is not automatically valid for waffle.
...
- do not assume ba-family component structure can be copied directly into waffle
...
Scripts matter because they are where BlueArchive crosses the boundary from shell UI into the user system.
...
If you change `sdata/`, you are usually changing how BlueArchive installs, updates, or maintains itself on user machines.
...
It matters for the user environment around BlueArchive, especially when the project applies or ships theming-related defaults.
...
## 4. Appearance is the visual contract for the ba family

`modules/common/Appearance.qml` is the theme and visual token singleton for the shared/ba side of the shell.
...
Most feature work in BlueArchive follows this relationship chain:
...
- many ba/shared modules at once
...
- ba/shared visual behavior -> usually `Appearance` plus ba/shared modules
...
BlueArchive scales by separating concerns.
