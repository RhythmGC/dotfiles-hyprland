...
BlueArchive includes automatic Discord/Vesktop theming that syncs with your wallpaper colors.
...
Features:
- Oxanium font (BlueArchive branding)
...
2. The theme is automatically installed to `~/.config/vesktop/themes/` during BlueArchive setup
...
```fish
# Regenerate theme
bash ~/.config/quickshell/ba/scripts/colors/system24_palette.sh

# Or trigger a full wallpaper refresh
~/.config/quickshell/ba/scripts/colors/switchwall.sh --noswitch
```
...
- Optional: place a local copy of System24 at `~/.config/vesktop/themes/system24.local.css` (same folder as the theme). If present, BlueArchive will import it first.

### Wrong colors
- Run `bash ~/.config/quickshell/ba/scripts/colors/system24_palette.sh` to regenerate
- Check `~/.local/state/quickshell/user/generated/palette.json` exists

### Debugging generation failures
- Run `bash ~/.config/quickshell/ba/scripts/colors/system24_palette.sh` in a terminal and check for errors
- If you're using preset themes (Settings), `apply-gtk-theme.sh` no longer suppresses Python errors, so `qs log -c ba` (or your shell logs) should show failures
