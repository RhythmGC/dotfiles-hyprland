...
BlueArchive — это весь твой рабочий стол. Панель сверху, док, уведомления, настройки, обои, всё. Это не тема, не конфиги для копипаста. Это полноценный шелл, работающий на Linux.

### Что нужно для запуска?

Композитор. Это штука, которая управляет окнами и выводит пиксели на экран. BlueArchive сделан для [Niri](https://github.com/YaLTeR/niri) (тайлинговый Wayland-композитор). Есть старый код для Hyprland со времён форка dots от end-4, но реально тестируется и используется Niri.
...
твои приложения
   ↓
BlueArchive (шелл: панель, сайдбары, док, уведомления, настройки...)
   ↓
...
Если что-то не работает, `ba doctor` чинит большинство проблем. Discord активный, если это не поможет. Не жди отполированного софта — это рис одного человека, который понравился другим.
...
- **QML**: декларативный язык UI от Qt, на нём написан BlueArchive
- **Material You**: система цветов Google, генерирует палитры из картинок (так работает авто-тематизация)
- **ba / waffle**: два стиля панелей. ba = Material Design вайбы, waffle = Windows 11 вайбы. `Super+Shift+W` переключает
...
<details open>
<summary><b>Material ba</b> — плавающая панель, сайдбары, эстетика Material Design</summary>
...
- **Material ba** — плавающая панель, сайдбары, док, 5 визуальных стилей (material, cards, aurora, ba, angel)
...
- **5 визуальных стилей** — Material (сплошной), Cards, Aurora (стеклянное размытие), BlueArchive (в духе TUI), Angel (нео-брутализм)
...
### Сайдбары и виджеты (Material ba)
...
- **Авто-обновления** — `ba update` с откатом, миграциями и сохранением пользовательских изменений
...
Установщик разберётся с зависимостями, системным конфигом, тематизацией — всем. После установки запустите `ba run` или перелогиньтесь.

```bash
ba run                        # запустить шелл
ba settings                   # открыть GUI настроек
ba logs                       # проверить логи
ba doctor                     # автодиагностика и исправление
ba update                     # pull + миграции + перезапуск
```
...
| Метод | Команда |
|--------|---------|
| Системная установка | `sudo make install && ba run` |
| Меню TUI | `./setup` |
...
15 обоев идут в комплекте. Больше — в [BlueArchive wallpapers](https://github.com/RhythmGC/dotfiles-hyprland/tree/main/.config/quickshell/ba/defaults), подборка, хорошо работающая с пайплайном Material You.
...
```bash
ba logs                       # проверьте логи — ответ обычно там
ba restart                    # перезапустить шелл
ba repair                     # doctor + перезапуск + проверка логов
...
- [**end-4**](https://github.com/end-4/dots-hyprland) — оригинальный baOS для Hyprland
...
<p align="center">
  <a href="https://github.com/RhythmGC/dotfiles-hyprland/graphs/contributors">Участники</a> &bull;
