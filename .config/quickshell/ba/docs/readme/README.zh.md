...
BlueArchive 是你的整个桌面。顶部的栏、Dock、通知、设置、壁纸，全部。不是主题，不是复制粘贴的配置文件。是一个运行在 Linux 上的完整 Shell。

### 运行需要什么？

一个合成器。就是管理窗口、把像素画到屏幕上的东西。BlueArchive 是为 [Niri](https://github.com/YaLTeR/niri)（一个平铺式 Wayland 合成器）做的。有一些从 end-4 的 dots fork 过来的 Hyprland 旧代码，但真正在测试和使用的是 Niri。
...
你的应用
   ↓
BlueArchive（Shell：栏、侧边栏、Dock、通知、设置...）
   ↓
...
如果出了问题，`ba doctor` 能修大部分。Discord 很活跃，那个不行就问那边。别指望精雕细琢的软件——这是一个人的 rice，碰巧其他人也喜欢。
...
- **QML**：Qt 的声明式 UI 语言，BlueArchive 就是用这个写的
- **Material You**：Google 的配色系统，从图片生成调色板（自动主题就是这么工作的）
- **ba / waffle**：两种面板风格。ba = Material Design 风，waffle = Windows 11 风。`Super+Shift+W` 切换
...
<details open>
<summary><b>Material ba</b> — 浮动栏、侧边栏、Material Design 风格</summary>
...
- **Material ba** — 浮动栏、侧边栏、Dock、5 种视觉风格（material、cards、aurora、ba、angel）
...
- **5 种视觉风格** — Material（纯色）、Cards、Aurora（毛玻璃）、BlueArchive（TUI 风格）、Angel（新残酷主义）
...
安装程序处理依赖、系统配置、主题 — 一切。安装后运行 `ba run` 启动 Shell，或注销后重新登录。

```bash
ba run                        # 启动 Shell
ba settings                   # 打开设置 GUI
ba logs                       # 查看运行日志
ba doctor                     # 自动诊断和修复
ba update                     # 拉取 + 迁移 + 重启
```
...
内置 15 张壁纸。更多壁纸请查看 [BlueArchive wallpapers](https://github.com/RhythmGC/dotfiles-hyprland/tree/main/.config/quickshell/ba/defaults) — 精选合集，与 Material You 管线完美配合。
...
- [**end-4**](https://github.com/end-4/dots-hyprland) — 原始的 baOS（Hyprland 版）
...
<p align="center">
  <a href="https://github.com/RhythmGC/dotfiles-hyprland/graphs/contributors">贡献者</a> &bull;
