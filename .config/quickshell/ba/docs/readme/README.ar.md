...
BlueArchive هو سطح المكتب كامل. الشريط فوق، الدوك، الإشعارات، الإعدادات، الخلفيات، كل شي. مو ثيم، مو ملفات dotfiles تنسخها. شل كامل يشتغل على لينكس.

### شو أحتاج عشان أشغله؟

كومبوزيتر. هذا اللي يدير النوافذ ويحط البكسلات على الشاشة. BlueArchive مصمم لـ [Niri](https://github.com/YaLTeR/niri) (كومبوزيتر Wayland تايلنق). في كود قديم من Hyprland من أيام ما كان فورك من dots الـ end-4، بس Niri هو اللي فعلياً أجربه وأستخدمه.
...
تطبيقاتك
   ↓
BlueArchive (شل: بار، سايدبار، دوك، إشعارات، إعدادات...)
   ↓
...
إذا شي ما اشتغل، `ba doctor` يصلح أغلب الأشياء. إذا ما نفع، الـ Discord نشط. لا تتوقع سوفتوير مصقول، هذا rice شخص واحد عجب ناس ثانيين.
...
- **QML**: لغة UI من Qt، BlueArchive مكتوب فيها
- **Material You**: نظام ألوان قوقل اللي يسوي باليتات من الصور (كذا يشتغل الـ auto-theming)
- **ba / waffle**: ستايلين للبانل. ba = فايب Material Design، waffle = فايب Windows 11. `Super+Shift+W` يبدل بينهم
...
<details open>
<summary><b>Material ba</b> — شريط عائم، أشرطة جانبية، جمالية Material Design</summary>
...
- **Material ba** — شريط عائم، أشرطة جانبية، رصيف، 5 أنماط بصرية (material، cards، aurora، ba، angel)
...
- **5 أنماط بصرية** — Material (صلب)، Cards، Aurora (ضبابية زجاجية)، BlueArchive (مستوحى من TUI)، Angel (وحشية جديدة)
...
### الأشرطة الجانبية والودجات (Material ba)
...
- **تحديثات تلقائية** — `ba update` مع تراجع وترحيل وحفظ تغييرات المستخدم
...
يتعامل المُثبّت مع التبعيات، إعداد النظام، السمات — كل شيء. بعد التثبيت، شغّل `ba run` أو سجّل الخروج وأعد تسجيل الدخول.

<div dir="ltr">

```bash
ba run                        # تشغيل الواجهة
ba settings                   # فتح إعدادات واجهة رسومية
ba logs                       # فحص سجلات وقت التشغيل
ba doctor                     # تشخيص وإصلاح تلقائي
ba update                     # سحب + ترحيل + إعادة تشغيل
```
...
| الطريقة | الأمر |
|--------|---------|
| تثبيت نظام | `sudo make install && ba run` |
| قائمة TUI | `./setup` |
...
15 خلفية مضمنة. للمزيد، تحقق من [BlueArchive wallpapers](https://github.com/RhythmGC/dotfiles-hyprland/tree/main/.config/quickshell/ba/defaults) — مجموعة منسقة تعمل جيداً مع خط أنابيب Material You.
...
```bash
ba logs                       # فحص سجلات وقت التشغيل الأخيرة
ba restart                    # إعادة تشغيل وقت التشغيل النشط
ba repair                     # doctor + إعادة تشغيل + فحص سجلات مفلترة
...
- [**end-4**](https://github.com/end-4/dots-hyprland) — baOS الأصلي لـ Hyprland
...
<p align="center">
  <a href="https://github.com/RhythmGC/dotfiles-hyprland/graphs/contributors">المساهمون</a> &bull;
