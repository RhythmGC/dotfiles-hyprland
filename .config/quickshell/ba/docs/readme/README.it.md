...
BlueArchive è tutto il tuo desktop. La barra in alto, il dock, notifiche, impostazioni, sfondi, tutto. Non è un tema, non sono dotfiles da copiare. È una shell completa che gira su Linux.

### Cosa serve per farlo girare?

Un compositor. È quello che gestisce le finestre e mette i pixel sullo schermo. BlueArchive è fatto per [Niri](https://github.com/YaLTeR/niri) (un compositor Wayland a tiling). C'è del vecchio codice Hyprland da quando era un fork dei dots di end-4, ma Niri è quello che testo e uso davvero.
...
le tue app
   ↓
BlueArchive (shell: barra, sidebar, dock, notifiche, impostazioni...)
   ↓
...
Se qualcosa non funziona, `ba doctor` sistema la maggior parte. Se non basta, Discord è attivo. Non aspettarti software rifinito, questo è il rice di una persona che è piaciuto ad altri.
...
- **QML**: linguaggio UI dichiarativo di Qt, BlueArchive è scritto in questo
- **Material You**: sistema colori Google che genera palette da immagini (così funziona l'auto-theming)
- **ba / waffle**: i due stili di pannello. ba = vibes Material Design, waffle = vibes Windows 11. `Super+Shift+W` per cambiare
...
<details open>
<summary><b>Material ba</b> — barra flottante, barre laterali, estetica Material Design</summary>
...
- **Material ba** — barra flottante, barre laterali, dock, 5 stili visivi (material, cards, aurora, ba, angel)
...
- **5 stili visivi** — Material (pieno), Cards, Aurora (sfocatura vetro), BlueArchive (ispirato TUI), Angel (neo-brutalismo)
...
### Barre laterali e widget (Material ba)
...
- **Aggiornamenti automatici** — `ba update` con rollback, migrazioni e preservazione delle modifiche utente
...
L'installer gestisce dipendenze, configurazione di sistema, tematizzazione — tutto. Dopo l'installazione, esegui `ba run` per avviare la shell, o disconnettiti e riconnettiti.

```bash
ba run                        # avviare la shell
ba settings                   # aprire le impostazioni GUI
ba logs                       # controllare i log di runtime
ba doctor                     # auto-diagnosi e riparazione
ba update                     # pull + migrazioni + riavvio
```
...
| Metodo | Comando |
|--------|---------|
| Installazione di sistema | `sudo make install && ba run` |
| Menu TUI | `./setup` |
...
15 sfondi sono inclusi. Per altri, vedi [BlueArchive wallpapers](https://github.com/RhythmGC/dotfiles-hyprland/tree/main/.config/quickshell/ba/defaults) — una collezione curata che funziona bene con la pipeline Material You.
...
```bash
ba logs                       # controlla i log — la risposta di solito è lì
ba restart                    # riavvia la shell
ba repair                     # doctor + riavvio + controllo log filtrato
...
- [**end-4**](https://github.com/end-4/dots-hyprland) — baOS originale per Hyprland
...
<p align="center">
  <a href="https://github.com/RhythmGC/dotfiles-hyprland/graphs/contributors">Contributori</a> &bull;
