...
BlueArchive은 데스크톱 전체입니다. 위의 바, 독, 알림, 설정, 배경화면, 전부. 테마가 아니고, 복붙하는 dotfiles도 아닙니다. Linux에서 돌아가는 완전한 셸이에요.

### 실행하려면 뭐가 필요해요?

컴포지터. 창을 관리하고 화면에 픽셀을 그리는 거예요. BlueArchive은 [Niri](https://github.com/YaLTeR/niri) (타일링 Wayland 컴포지터)용으로 만들어졌어요. end-4의 dots에서 포크했을 때의 오래된 Hyprland 코드가 있지만, 실제로 테스트하고 쓰는 건 Niri입니다.
...
뭔가 안 되면 `ba doctor`가 대부분 고쳐요. 그래도 안 되면 Discord가 활발해요. 완성된 소프트웨어를 기대하지 마세요, 이건 다른 사람들이 좋아하게 된 한 사람의 rice입니다.
...
- **QML**: Qt의 선언적 UI 언어, BlueArchive은 이걸로 작성됨
- **Material You**: 이미지에서 팔레트 생성하는 Google 색상 시스템 (자동 테마가 이렇게 작동함)
- **ba / waffle**: 두 패널 스타일. ba = Material Design 느낌, waffle = Windows 11 느낌. `Super+Shift+W`로 전환
...
<details open>
<summary><b>Material ba</b> — 플로팅 바, 사이드바, Material Design 미학</summary>
...
- **Material ba** — 플로팅 바, 사이드바, 독, 5가지 비주얼 스타일 (material, cards, aurora, ba, angel)
...
- **5가지 비주얼 스타일** — Material (솔리드), Cards, Aurora (유리 블러), BlueArchive (TUI 영감), Angel (네오 브루탈리즘)
...
- **자동 업데이트** — `ba update`, 롤백·마이그레이션·사용자 변경 보존 포함
...
인스톨러가 의존성, 시스템 설정, 테마 — 모든 것을 처리합니다. 설치 후 `ba run`으로 셸을 시작하거나 로그아웃 후 다시 로그인하세요.

```bash
ba run                        # 셸 시작
ba settings                   # 설정 GUI 열기
ba logs                       # 런타임 로그 확인
ba doctor                     # 자동 진단 및 수정
ba update                     # pull + 마이그레이션 + 재시작
```
...
| 방법 | 명령 |
|--------|---------|
| 시스템 설치 | `sudo make install && ba run` |
| TUI 메뉴 | `./setup` |
| 롤백 | `./setup rollback` |
...
15개 배경화면이 기본 포함됩니다. 더 필요하면 [BlueArchive wallpapers](https://github.com/RhythmGC/dotfiles-hyprland/tree/main/.config/quickshell/ba/defaults)를 확인하세요 — Material You 파이프라인과 잘 맞는 큐레이션 컬렉션입니다.
...
```bash
ba logs                       # 최근 런타임 로그 확인
ba restart                    # 활성 런타임 재시작
ba repair                     # doctor + 재시작 + 필터링된 로그 확인
...
- [**end-4**](https://github.com/end-4/dots-hyprland) — Hyprland용 오리지널 baOS
...
<p align="center">
  <a href="https://github.com/RhythmGC/dotfiles-hyprland/graphs/contributors">기여자</a> &bull;
