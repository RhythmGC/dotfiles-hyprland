...
BlueArchive ist dein kompletter Desktop. Die Leiste oben, das Dock, Benachrichtigungen, Einstellungen, Hintergründe, alles. Kein Theme, keine Dotfiles zum Kopieren. Eine vollständige Shell die auf Linux läuft.

### Was brauche ich?

Einen Compositor. Das ist das Ding das deine Fenster verwaltet und Pixel auf den Bildschirm bringt. BlueArchive ist für [Niri](https://github.com/YaLTeR/niri) gebaut (ein Tiling Wayland Compositor). Es gibt alten Hyprland-Code von als das noch ein Fork von end-4s dots war, aber Niri ist was ich wirklich teste und benutze.
...
deine Apps
   ↓
BlueArchive (Shell: Leiste, Sidebars, Dock, Benachrichtigungen, Einstellungen...)
   ↓
...
Wenn was nicht funktioniert, `ba doctor` behebt das meiste. Discord ist aktiv wenn das nicht hilft. Erwarte keine polierte Software, das ist der Rice einer Person den andere mochten.
...
- **QML**: Qts deklarative UI-Sprache, BlueArchive ist darin geschrieben
- **Material You**: Googles Farbsystem das Paletten aus Bildern generiert (so funktioniert das Auto-Theming)
- **ba / waffle**: die zwei Panel-Stile. ba = Material Design Vibes, waffle = Windows 11 Vibes. `Super+Shift+W` wechselt
...
<details open>
<summary><b>Material ba</b> — schwebende Leiste, Seitenleisten, Material-Design-Ästhetik</summary>
...
- **Material ba** — schwebende Leiste, Seitenleisten, Dock, 5 visuelle Stile (material, cards, aurora, ba, angel)
...
- **5 visuelle Stile** — Material (einfarbig), Cards, Aurora (Glasunschärfe), BlueArchive (TUI-inspiriert), Angel (Neo-Brutalismus)
...
### Seitenleisten und Widgets (Material ba)
...
- **Auto-Updates** — `ba update` mit Rollback, Migrationen und Erhalt von Benutzeränderungen
...
Der Installer kümmert sich um Abhängigkeiten, Systemkonfiguration, Thematisierung — alles. Nach der Installation `ba run` starten oder ab- und wieder anmelden.

```bash
ba run                        # Shell starten
ba settings                   # Einstellungen-GUI öffnen
ba logs                       # Runtime-Logs prüfen
ba doctor                     # Auto-Diagnose und Reparatur
ba update                     # Pull + Migrationen + Neustart
```
...
| Methode | Befehl |
|--------|---------|
| Systeminstallation | `sudo make install && ba run` |
| TUI-Menü | `./setup` |
...
15 Hintergrundbilder sind enthalten. Für mehr siehe [BlueArchive wallpapers](https://github.com/RhythmGC/dotfiles-hyprland/tree/main/.config/quickshell/ba/defaults) — eine kuratierte Sammlung, die gut mit der Material-You-Pipeline funktioniert.
...
```bash
ba logs                       # Logs prüfen — die Antwort ist meist dort
ba restart                    # Shell neu starten
ba repair                     # Doctor + Neustart + gefilterte Log-Prüfung
...
- [**end-4**](https://github.com/end-4/dots-hyprland) — Original baOS für Hyprland
...
<p align="center">
  <a href="https://github.com/RhythmGC/dotfiles-hyprland/graphs/contributors">Mitwirkende</a> &bull;
