...
BlueArchive es todo tu escritorio. La barra de arriba, el dock, notificaciones, configuración, wallpapers, todo. No es un theme, no son dotfiles que pegás. Es un shell completo que corre en Linux.

### ¿Qué necesito para usarlo?

Un compositor. Es lo que maneja tus ventanas y pone los pixeles en pantalla. BlueArchive está hecho para [Niri](https://github.com/YaLTeR/niri) (un compositor Wayland de tiling). Hay código viejo de Hyprland de cuando esto era un fork de los dots de end-4, pero Niri es lo que realmente uso y testeo.
...
tus apps
   ↓
BlueArchive (shell: barra, sidebars, dock, notificaciones, settings...)
   ↓
...
Si algo no anda, `ba doctor` arregla la mayoría. El Discord está activo si eso no ayuda. No esperes software pulido, esto es el rice de una persona que a otros les gustó.
...
- **QML**: lenguaje declarativo de UI de Qt, en lo que está escrito BlueArchive
- **Material You**: sistema de colores de Google que hace paletas de imágenes (así funciona el auto-theming)
- **ba / waffle**: los dos estilos de panel. ba = onda Material Design, waffle = onda Windows 11. `Super+Shift+W` cambia entre ellos
...
<details open>
<summary><b>Material ba</b> — barra flotante, sidebars, estética Material Design</summary>
...
- **Material ba** — barra flotante, sidebars, dock, 5 estilos visuales (material, cards, aurora, ba, angel)
...
- **5 estilos visuales** — Material (sólido), Cards, Aurora (blur de vidrio), BlueArchive (inspirado en TUI), Angel (neo-brutalismo)
...
### Sidebars y widgets (Material ba)
...
- **Auto-updates** — `ba update` con rollback, migraciones y preservación de cambios del usuario
...
El instalador maneja dependencias, config del sistema, tematización — todo. Después de instalar, ejecutá `ba run` para iniciar el shell, o cerrá sesión y volvé a entrar.

```bash
ba run                        # iniciar el shell
ba settings                   # abrir configuración GUI
ba logs                       # ver logs del runtime
ba doctor                     # auto-diagnosticar y arreglar
ba update                     # pull + migrar + reiniciar
```
...
| Método | Comando |
|--------|---------|
| Instalación de sistema | `sudo make install && ba run` |
| Menú TUI | `./setup` |
...
15 wallpapers vienen incluidos. Para más, mirá [BlueArchive wallpapers](https://github.com/RhythmGC/dotfiles-hyprland/tree/main/.config/quickshell/ba/defaults) — una colección curada que funciona bien con el pipeline de Material You.
...
```bash
ba logs                       # revisá los logs — la respuesta suele estar ahí
ba restart                    # reiniciar el shell
ba repair                     # doctor + restart + chequeo de logs filtrado
...
- [**end-4**](https://github.com/end-4/dots-hyprland) — baOS original para Hyprland
...
<p align="center">
  <a href="https://github.com/RhythmGC/dotfiles-hyprland/graphs/contributors">Contribuidores</a> &bull;
