<p align="center">
  <img src="https://github.com/user-attachments/assets/da6beb4a-ccee-40ba-a372-5eea77b595f8" alt="BlueArchive" width="800">
</p>

<h1 align="center">BlueArchive</h1>
...
<p align="center">
  <a href="https://github.com/RhythmGC/dotfiles-hyprland/releases"><img src="https://img.shields.io/badge/version-2.27.0-blue?style=flat-square" alt="Version"></a>
  <a href="https://github.com/RhythmGC/dotfiles-hyprland/stargazers"><img src="https://img.shields.io/github/stars/RhythmGC/dotfiles-hyprland?style=flat-square" alt="Stars"></a>
...
BlueArchive c'est tout ton bureau. La barre en haut, le dock, les notifications, les paramètres, les fonds d'écran, tout. C'est pas un thème, c'est pas des dotfiles à copier-coller. C'est un shell complet qui tourne sur Linux.

### Qu'est-ce qu'il faut ?

Un compositeur. C'est ce qui gère tes fenêtres et met les pixels à l'écran. BlueArchive est fait pour [Niri](https://github.com/YaLTeR/niri) (un compositeur Wayland en tiling). Y'a du vieux code Hyprland de quand c'était un fork des dots de end-4, mais c'est Niri que je teste et utilise vraiment.
...
tes apps
   ↓
BlueArchive (shell : barre, sidebars, dock, notifications, paramètres...)
   ↓
...
Si un truc marche pas, `ba doctor` règle la plupart des problèmes. Le Discord est actif si ça suffit pas. Attends pas un logiciel fini, c'est le rice d'une personne que d'autres ont aimé.
...
- **QML** : langage déclaratif UI de Qt, BlueArchive est écrit dedans
...
- **ba / waffle** : les deux styles de panneaux. ba = vibes Material Design, waffle = vibes Windows 11. `Super+Shift+W` pour changer
...
<details open>
<summary><b>Material ba</b> — barre flottante, barres latérales, esthétique Material Design</summary>
...
- **Material ba** — barre flottante, barres latérales, dock, 5 styles visuels (material, cards, aurora, ba, angel)
...
- **5 styles visuels** — Material (uni), Cards, Aurora (flou de verre), BlueArchive (inspiré TUI), Angel (néo-brutalisme)
...
### Barres latérales et widgets (Material ba)
...
- **Mises à jour auto** — `ba update` avec retour arrière, migrations et préservation des modifications utilisateur
...
L'installateur gère les dépendances, la config système, la thématisation — tout. Après l'installation, lancez `ba run` pour démarrer le shell, ou déconnectez-vous et reconnectez-vous.

```bash
ba run                        # lancer le shell
ba settings                   # ouvrir les paramètres GUI
ba logs                       # vérifier les logs d'exécution
ba doctor                     # auto-diagnostic et réparation
ba update                     # pull + migrations + redémarrage
```
...
| Méthode | Commande |
|--------|---------|
| Installation système | `sudo make install && ba run` |
| Menu TUI | `./setup` |
...
15 fonds d'écran sont inclus. Pour en avoir plus, consultez [BlueArchive wallpapers](https://github.com/RhythmGC/dotfiles-hyprland/tree/main/.config/quickshell/ba/defaults) — une collection qui fonctionne bien avec le pipeline Material You.
...
```bash
ba logs                       # vérifiez les logs — la réponse est souvent là
ba restart                    # redémarrer le shell
ba repair                     # doctor + redémarrage + vérification de logs filtrée
...
- [**end-4**](https://github.com/end-4/dots-hyprland) — baOS original pour Hyprland
...
<p align="center">
  <a href="https://github.com/RhythmGC/dotfiles-hyprland/graphs/contributors">Contributeurs</a> &bull;
