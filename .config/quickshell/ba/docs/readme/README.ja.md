...
BlueArchive はデスクトップ全体です。上のバー、ドック、通知、設定、壁紙、全部。テーマじゃない、コピペする設定ファイルでもない。Linux で動く完全なシェルです。

### 動かすのに何が必要？

コンポジター。ウィンドウを管理して画面にピクセルを描く部分。BlueArchive は [Niri](https://github.com/YaLTeR/niri)（タイリング Wayland コンポジター）向けに作られています。end-4 の dots からフォークした時の古い Hyprland コードもあるけど、実際にテストして使ってるのは Niri。
...
あなたのアプリ
   ↓
BlueArchive（シェル：バー、サイドバー、ドック、通知、設定...）
   ↓
...
何か動かなかったら `ba doctor` で大体直る。それでダメなら Discord が活発。洗練されたソフトを期待しないで、これは一人の rice で、たまたま他の人も気に入っただけ。
...
- **QML**：Qt の宣言的 UI 言語、BlueArchive はこれで書かれてる
- **Material You**：Google の配色システム、画像からパレットを生成（自動テーマの仕組み）
- **ba / waffle**：2つのパネルスタイル。ba = Material Design 風、waffle = Windows 11 風。`Super+Shift+W` で切り替え
...
<details open>
<summary><b>Material ba</b> — フローティングバー、サイドバー、Material Design 美学</summary>
...
- **Material ba** — フローティングバー、サイドバー、ドック、5つのビジュアルスタイル（material、cards、aurora、ba、angel）
...
- **5つのビジュアルスタイル** — Material（ソリッド）、Cards、Aurora（ガラスブラー）、BlueArchive（TUI 風）、Angel（ネオブルータリズム）
...
- **自動アップデート** — `ba update`、ロールバック・マイグレーション・ユーザー変更保持付き
...
インストーラーが依存関係、システム設定、テーマ化 — すべて処理します。インストール後、`ba run` でシェルを起動するか、ログアウトして再ログインしてください。

```bash
ba run                        # シェルを起動
ba settings                   # 設定 GUI を開く
ba logs                       # ランタイムログを確認
ba doctor                     # 自動診断と修復
ba update                     # pull + マイグレーション + 再起動
```
...
| 方法 | コマンド |
|--------|---------|
| システムインストール | `sudo make install && ba run` |
| TUI メニュー | `./setup` |
...
15枚の壁紙が同梱されています。さらに欲しい場合は [BlueArchive wallpapers](https://github.com/RhythmGC/dotfiles-hyprland/tree/main/.config/quickshell/ba/defaults) をチェック — Material You パイプラインと相性の良いキュレーションコレクションです。
...
```bash
ba logs                       # 最近のランタイムログを確認
ba restart                    # アクティブなランタイムを再起動
ba repair                     # doctor + 再起動 + フィルタ済みログチェック
...
- [**end-4**](https://github.com/end-4/dots-hyprland) — Hyprland 向けオリジナル baOS
...
<p align="center">
  <a href="https://github.com/RhythmGC/dotfiles-hyprland/graphs/contributors">コントリビューター</a> &bull;
