...
BlueArchive é o teu desktop inteiro. A barra no topo, o dock, notificações, configurações, wallpapers, tudo. Não é um tema, não são dotfiles pra copiar e colar. É um shell completo que roda no Linux.

### O que preciso pra usar?

Um compositor. É o que gerencia janelas e coloca pixels na tela. BlueArchive foi feito pro [Niri](https://github.com/YaLTeR/niri) (um compositor Wayland de tiling). Tem código velho do Hyprland de quando isso era um fork dos dots do end-4, mas o que realmente testo e uso é Niri.
...
teus apps
   ↓
BlueArchive (shell: barra, sidebars, dock, notificações, settings...)
   ↓
...
Se algo não funcionar, `ba doctor` arruma a maioria. Discord tá ativo se isso não resolver. Não espera software polido, isso é o rice de uma pessoa que outros curtiram.
...
- **QML**: linguagem declarativa de UI do Qt, BlueArchive é escrito nisso
- **Material You**: sistema de cores do Google que gera paletas de imagens (assim funciona o auto-theming)
- **ba / waffle**: os dois estilos de painel. ba = vibe Material Design, waffle = vibe Windows 11. `Super+Shift+W` alterna
...
<details open>
<summary><b>Material ba</b> — barra flutuante, sidebars, estética Material Design</summary>
...
- **Material ba** — barra flutuante, sidebars, dock, 5 estilos visuais (material, cards, aurora, ba, angel)
...
- **5 estilos visuais** — Material (sólido), Cards, Aurora (blur de vidro), BlueArchive (inspirado em TUI), Angel (neo-brutalismo)
...
### Sidebars e widgets (Material ba)
...
- **Atualizações automáticas** — `ba update` com rollback, migrações e preservação de mudanças do usuário
...
O instalador cuida de dependências, configuração do sistema, tematização — tudo. Após instalar, rode `ba run` para iniciar o shell, ou faça logout e login novamente.

```bash
ba run                        # iniciar o shell
ba settings                   # abrir configurações GUI
ba logs                       # verificar logs de runtime
ba doctor                     # auto-diagnosticar e corrigir
ba update                     # pull + migrações + reiniciar
```
...
| Método | Comando |
|--------|---------|
| Instalação de sistema | `sudo make install && ba run` |
| Menu TUI | `./setup` |
...
15 wallpapers vêm incluídos. Para mais, confira [BlueArchive wallpapers](https://github.com/RhythmGC/dotfiles-hyprland/tree/main/.config/quickshell/ba/defaults) — uma coleção curada que funciona bem com o pipeline Material You.
...
```bash
ba logs                       # verifique os logs — a resposta geralmente está lá
ba restart                    # reiniciar o shell
ba repair                     # doctor + reinício + verificação de logs filtrada
