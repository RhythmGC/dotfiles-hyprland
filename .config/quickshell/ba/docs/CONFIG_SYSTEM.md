...
How configuration works in BlueArchive, from the user's perspective and from the code side.

## For users

Everything is configurable through the graphical Settings UI. Open it with `Super+,` or `ba settings`. You should never need to edit the config file by hand.
...
(The directory name is a legacy artifact from when BlueArchive was called baOS. `~/.config/baOS` is symlinked to it.)
...
`bar.layout` controls the modular ba bar:
...
- **Material ba**: `Super+,` opens an overlay settings panel (lives in `modules/settings/`)
