...
How BlueArchive is put together, from the highest level down to the part that matters for your question.

## The 30-second version

BlueArchive is a single-process QML application that runs inside Quickshell. There's no backend daemon, no IPC server, no separate process for services. Everything (the bar, sidebars, notifications, audio control, compositor IPC, config management) lives in one QML runtime talking to system services through D-Bus, sockets, and subprocess calls.

```
Your apps
   |
BlueArchive shell (bar, panels, notifications, settings, everything)
   |
...
4. Loads either `ShellBaPanels.qml` or `ShellWafflePanels.qml` based on the panel family config
...
**Material ba** uses Material Design language with 5 style variants (material, cards, aurora, ba, angel). Bar at the top. Sidebars from the edges. Overview launcher.
...
- **User file**: `~/.config/baOS/config.json`
...
BlueArchive ships as a git repo with an interactive installer:
...
- **Repo-sync**: `./setup install` syncs the repo to `~/.config/quickshell/ba/`
- **Package-managed**: `make install` copies to `/usr/share/quickshell/ba/`

Updates: `./setup update` or `ba update` (git pull + sync + migrate + restart).
