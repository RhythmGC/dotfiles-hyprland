...
```bash
ba wallpapers set /path/to/image.jpg
```
...
Set an interval and BlueArchive cycles through wallpapers in a directory automatically. Configurable per-directory with optional color regeneration on each change.
...
```bash
ba wallpapers set <path>          # Set wallpaper and regenerate colors
ba wallpapers get                 # Get current wallpaper path
ba wallpapers random <directory>  # Set a random wallpaper from directory
```
