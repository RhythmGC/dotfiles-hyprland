...
Normally, BlueArchive extracts colors from your wallpaper using Material You. Presets skip that step and inject a complete Material 3 color palette directly. The palette propagates to external apps (GTK, terminals, Firefox, etc.) the same way wallpaper colors do.

Apply presets from Settings > Appearance > Theme, or via IPC:

```bash
ba theme setPreset gruvbox-dark
ba theme setPreset catppuccin-mocha
ba theme auto                        # back to wallpaper-based
```
