...
- **Getting Started**
  Install BlueArchive, run the setup wizard, get a shell on screen.
  [Installation](INSTALL.md)

- **Panel Families**
  Material ba (5 styles) and Waffle (Windows 11). How they work, how to switch.
...
| Styles        | material, cards, aurora, ba, angel | Fluent                        |

## Common commands

```bash
ba run            # start the shell
ba restart        # restart running instance
ba settings       # open settings UI
ba logs --full    # view full Quickshell logs
ba doctor         # health checks and auto-fix
...
| **Bash control plane** | `scripts/ba` (2400+ lines). Launcher, service lifecycle, IPC routing, diagnostics. |
...
| `scripts/ba` | CLI launcher and IPC router. |
| `scripts/colors/` | Wallpaper-to-color pipeline. |

---

This documentation reflects the `main` branch. For the repo-level overview, see [ARCHITECTURE.md](https://github.com/RhythmGC/dotfiles-hyprland/blob/main/.config/quickshell/ba/ARCHITECTURE.md).
