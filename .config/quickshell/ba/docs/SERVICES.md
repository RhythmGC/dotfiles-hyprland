...
> Only some services expose an IPC handler. The real IPC targets are listed where they exist; if a service has no `IPC target:` note, it has no `IpcHandler` and is not callable from the `ba` CLI. To regenerate the authoritative list: `grep -rl IpcHandler services/ | xargs grep -oP 'target:\s*"\K[^"]+'`.
...
| **ShellUpdates** | BlueArchive update checker. Git-based, tracks commits behind remote. IPC target: `shellUpdate`. |
