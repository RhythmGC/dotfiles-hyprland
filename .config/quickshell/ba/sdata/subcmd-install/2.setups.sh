...
function setup_super_daemon(){
  ...
  local daemon_src="${REPO_ROOT}/scripts/daemon/ba_super_overview_daemon.py"
  local service_src="${REPO_ROOT}/scripts/systemd/ba-super-overview.service"
  local daemon_dst="${HOME}/.local/bin/ba_super_overview_daemon.py"
  local service_dst="${XDG_CONFIG_HOME}/systemd/user/ba-super-overview.service"
  ...
  if [[ -n "${DBUS_SESSION_BUS_ADDRESS}" ]]; then
    v systemctl --user daemon-reload
    v systemctl --user enable ba-super-overview.service --now
  else
    log_warning "Not in graphical session. Enable later with:"
    echo "  systemctl --user enable ba-super-overview.service --now"
  fi
...
function disable_super_daemon_if_present(){
  ...
  local service_dst="${systemd_user_dir}/ba-super-overview.service"
  ...
  if [[ -n "${DBUS_SESSION_BUS_ADDRESS}" && -f "${service_dst}" ]]; then
    ...
    echo "  systemctl --user disable --now ba-super-overview.service"
  fi
...
function setup_desktop_settings(){
  ...
  # Keep default icon theme aligned with BlueArchive defaults/config and installer payload.
...
# Disabled by default in favor of Mod+Space ba overview.
# To install anyway, set BA_ENABLE_SUPER_DAEMON=1 in the environment.
if [[ "${BA_ENABLE_SUPER_DAEMON:-0}" == "1" ]]; then
