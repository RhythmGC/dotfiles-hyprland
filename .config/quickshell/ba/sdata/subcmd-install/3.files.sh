# Config file installation for BlueArchive
...
BA_LAUNCHER_PATH="${XDG_BIN_HOME}/ba"
BA_APPLICATIONS_DIR="${XDG_DATA_HOME}/applications"
BA_ICON_DIR="${XDG_DATA_HOME}/icons/hicolor/scalable/apps"
...
#####################################################################################
# Install Quickshell config (ba)
#####################################################################################
case "${SKIP_QUICKSHELL}" in
  true) sleep 0;;
  *)
    tui_info "Installing Quickshell ba config..."

    # The ba QML code is in the root of this repo, not in dots/
    # We copy it to ~/.config/quickshell/ba/
    BA_SOURCE="${REPO_ROOT}"
    ...
    if [[ "${IS_UPDATE}" == "true" ]]; then
      ...
      create_update_backup "$BA_TARGET" >/dev/null
    fi

    # Generate manifest BEFORE syncing (to know what should exist)
    log_info "Generating file manifest..."
    generate_manifest "$BA_SOURCE" "${BA_TARGET}/.ba-manifest.new"

    # Copy all .qml files from root (auto-detect, no manual list needed)
    for qml_file in "${BA_SOURCE}"/*.qml; do
      if [[ -f "$qml_file" ]]; then
        install_file "$qml_file" "${BA_TARGET}/$(basename "$qml_file")"
      fi
    done

    runtime_root_manifest="${BA_SOURCE}/sdata/runtime-root-files.txt"
    if [[ -f "$runtime_root_manifest" ]]; then
      while IFS= read -r runtime_file; do
        [[ -n "$runtime_file" ]] || continue
        if [[ -f "${BA_SOURCE}/${runtime_file}" ]]; then
          install_file "${BA_SOURCE}/${runtime_file}" "${BA_TARGET}/${runtime_file}"
        fi
      done < "$runtime_root_manifest"
    fi

    # Copy runtime payload directories
    runtime_dirs_manifest="${BA_SOURCE}/sdata/runtime-payload-dirs.txt"
    if [[ -f "$runtime_dirs_manifest" ]]; then
      while IFS= read -r dir; do
        [[ -n "$dir" ]] || continue
        if [[ -d "${BA_SOURCE}/${dir}" ]]; then
          install_dir__sync "${BA_SOURCE}/${dir}" "${BA_TARGET}/${dir}"
        fi
      done < "$runtime_dirs_manifest"
    fi

    # Finalize manifest
    mv "${BA_TARGET}/.ba-manifest.new" "${BA_TARGET}/.ba-manifest"

    # Cleanup orphan files (files that no longer exist in repo)
    if [[ "${IS_UPDATE}" == "true" ]]; then
      log_info "Cleaning up orphan files..."
      cleanup_orphans "$BA_TARGET" "${BA_TARGET}/.ba-manifest"
    fi

    # Fix script permissions
    log_info "Setting script permissions..."
    find "$BA_TARGET/scripts" \( -name "*.sh" -o -name "*.fish" -o -name "*.py" \) -exec chmod +x {} \; 2>/dev/null || true
    [[ -f "${BA_TARGET}/setup" ]] && chmod +x "${BA_TARGET}/setup"

    if [[ -f "${REPO_ROOT}/scripts/ba" ]]; then
      install_file "${REPO_ROOT}/scripts/ba" "${BA_LAUNCHER_PATH}"
      chmod +x "${BA_LAUNCHER_PATH}"
    ...
    local _service_asset="${REPO_ROOT}/assets/systemd/ba.service"
    local _service_target="${_service_dir}/ba.service"
    ...
    if [[ -f "$_service_asset" ]]; then
      ...
      if [[ -f "$_service_target" ]]; then
        ...
        if sync_user_ba_service_from_repo_if_present; then
          _service_refresh_status=0
          log_success "User ba.service refreshed"
        fi
      else
        # Fresh install: create service from template, rewriting ExecStart path
        local _tmp_svc="${XDG_CACHE_HOME:-$HOME/.cache}/ba.service.$$"
        local _launcher_escaped="${BA_LAUNCHER_PATH//&/\\&}"
        ...
        _service_refresh_status=0
        log_success "User ba.service installed"
      fi
    fi

    if [[ -f "$_service_target" ]]; then
      # Wire to compositor-specific wants so ba only starts under the correct
      ...
      if [[ -n "$_comp_target" ]]; then
        ...
        mkdir -p "$_wants_dir"
        ln -sf "${XDG_CONFIG_HOME}/systemd/user/ba.service" "$_wants_dir/ba.service"
        systemctl --user daemon-reload >/dev/null 2>&1 || true
        log_success "User ba.service enabled (wired to ${_comp_target})"
      else
        log_warning "No supported compositor detected (niri or Hyprland)"
        log_warning "ba.service not enabled — run 'ba service enable' from your compositor session"
      fi
    fi

    if [[ -f "${REPO_ROOT}/assets/icons/desktop-symbolic.svg" ]]; then
      install_file "${REPO_ROOT}/assets/icons/desktop-symbolic.svg" "${BA_ICON_DIR}/ba.svg"
      log_success "Launcher icon installed"
    fi

    if [[ -f "${REPO_ROOT}/assets/applications/ba.desktop" ]]; then
      BA_DESKTOP_TMP="${XDG_CACHE_HOME}/ba.desktop.$$"
      sed "s|^Exec=.*|Exec=${BA_LAUNCHER_PATH//&/\\&} service restart|" "${REPO_ROOT}/assets/applications/ba.desktop" > "${BA_DESKTOP_TMP}"
      install_file "${BA_DESKTOP_TMP}" "${BA_APPLICATIONS_DIR}/ba.desktop"
      rm -f "${BA_DESKTOP_TMP}"
      log_success "Shell desktop entry installed"
    fi

    if [[ -f "${REPO_ROOT}/assets/applications/ba-settings.desktop" ]]; then
      BA_SETTINGS_DESKTOP_TMP="${XDG_CACHE_HOME}/ba-settings.desktop.$$"
      sed "s|^Exec=.*|Exec=${BA_LAUNCHER_PATH//&/\\&} settings|" "${REPO_ROOT}/assets/applications/ba-settings.desktop" > "${BA_SETTINGS_DESKTOP_TMP}"
      install_file "${BA_SETTINGS_DESKTOP_TMP}" "${BA_APPLICATIONS_DIR}/ba-settings.desktop"
      rm -f "${BA_SETTINGS_DESKTOP_TMP}"
      log_success "Settings desktop entry installed"
    fi

    log_success "Quickshell ba config installed"
...
case "${SKIP_NIRI}" in
  ...
  *)
    ...
    # On first run, always install BlueArchive defaults (backup_clashing_targets already
    ...
    if [[ -f "$NIRI_CFG" ]]; then
      ...
      if pacman -Q plasma-integration &>/dev/null 2>&1 || \
         dpkg -l plasma-integration 2>/dev/null | grep -q '^ba' || \
      ...
      _launcher_path_escaped="${BA_LAUNCHER_PATH//&/\\&}"
      sed -i \
        -e 's|spawn "bash" "-lc" "exec \"\$(ba path)/scripts/launch-terminal.sh\""|spawn "'"${_launcher_path_escaped}"'" "terminal"|' \
        -e 's|spawn "bash" "-lc" "exec \"\$(ba path)/scripts/close-window.sh\""|spawn "'"${_launcher_path_escaped}"'" "close-window"|' \
        "$NIRI_BINDS_TARGET"
      sed -i \
        -e 's|spawn "ba" "|spawn "'"${_launcher_path_escaped}"'" "|g' \
        "$NIRI_BINDS_TARGET"
    fi
    ;;
esac
...
# ba-pixel-sddm theme (login screen matching ba lockscreen aesthetic)
...
if command -v sddm &>/dev/null; then
  if [[ "${INSTALL_FIRSTRUN}" == true ]]; then
    if [[ "${ask}" == "true" ]]; then
      tui_info "Recommended: install ba-pixel-sddm login theme (matches BlueArchive lockscreen)."
      if tui_confirm "Install ba-pixel-sddm now?" "yes"; then
        extras_install_sddm_theme "yes"
      else
        log_info "Skipping ba-pixel-sddm setup"
      fi
    else
...
if [[ -d "dots/.config/fish" ]]; then
  ...
  if [[ "${BA_SET_DEFAULT_SHELL:-}" == "true" ]]; then
...
# These are controlled by BlueArchive for theming - always overwrite
...
if [[ -d "dots/.config/vesktop/themes" ]]; then
  ...
  OLD_VESKTOP_THEMES=(
    "midnight-ba.theme.css"
    "dms-midnight.theme.css"
    "system24-ba.theme.css"
    "system24-palette.css"
    "ba-palette.css"
    "ba-system24.theme.css"
  )
...
#   PLUGINS_DIR="${XDG_CONFIG_HOME}/baOS/plugins"
...
if grep -q "BA_VENV" "${XDG_CONFIG_HOME}/niri/config.kdl" 2>/dev/null || \
   grep -q "BA_VENV_COMPAT" "${XDG_CONFIG_HOME}/niri/config.kdl" 2>/dev/null; then
    log_success "Environment variable configured in Niri config"
else
    log_warning "BA_VENV not found in Niri config"
fi
...
BASH_ENV_MARKER="# BlueArchive environment"
if [[ -f "$HOME/.bashrc" ]]; then
    # Clean up old markers
    sed -i '/BlueArchive-env.sh/d' "$HOME/.bashrc"
    # Remove any existing BlueArchive block
    sed -i "/${BASH_ENV_MARKER}/,/# end BlueArchive/d" "$HOME/.bashrc"
fi
...
cat > "${FISH_CONF_DIR}/ba-env.fish" << FEOF
# BlueArchive environment — auto-generated by setup install
...
if [[ -f "$HOME/.zshrc" ]]; then
    sed -i '/BlueArchive-env.sh/d' "$HOME/.zshrc"
    sed -i "/${BASH_ENV_MARKER}/,/# end BlueArchive/d" "$HOME/.zshrc"
    cat >> "$HOME/.zshrc" << ZEOF
...
#####################################################################################
# Ensure BA_TARGET is defined (in case SKIP_QUICKSHELL was set)
BA_TARGET="${BA_TARGET:-${XDG_CONFIG_HOME}/quickshell/ba}"
...
if [[ -d "${BA_TARGET}/assets/wallpapers" ]]; then
  mkdir -p "${USER_WALLPAPERS_DIR}"
  COPIED_COUNT=0
  for wallpaper in "${BA_TARGET}/assets/wallpapers"/*; do
    if [[ -f "$wallpaper" ]]; then
...
BA_WALLS_DEFAULT_OVERRIDE=""
if [[ "${INSTALL_FIRSTRUN}" == true && "${ask}" == "true" ]]; then
  if tui_confirm "Download and install BlueArchive-Walls wallpapers?" "no"; then
    EXTRAS_BA_WALLS_FIRST_IMAGE=""
    extras_install_ba_walls
    BA_WALLS_FIRST="${EXTRAS_BA_WALLS_FIRST_IMAGE}"
    if [[ -n "$BA_WALLS_FIRST" ]] && tui_confirm "Replace BlueArchive default wallpaper selection with an BlueArchive-Walls wallpaper?" "yes"; then
      BA_WALLS_DEFAULT_OVERRIDE="$BA_WALLS_FIRST"
      log_success "Default wallpaper source switched to BlueArchive-Walls"
    fi
  fi
fi
...
DEFAULT_WALLPAPER=""
if [[ -n "$BA_WALLS_DEFAULT_OVERRIDE" && -f "$BA_WALLS_DEFAULT_OVERRIDE" && -s "$BA_WALLS_DEFAULT_OVERRIDE" ]]; then
  DEFAULT_WALLPAPER="$BA_WALLS_DEFAULT_OVERRIDE"
fi
for candidate in \
...
if [[ "${INSTALL_FIRSTRUN}" == true && -n "${DEFAULT_WALLPAPER}" && -f "${DEFAULT_WALLPAPER}" ]]; then
  ...
  export BA_VENV="${XDG_STATE_HOME}/quickshell/.venv"
  export BA_VENV_COMPAT="${BA_VENV}"
  _init_python_cmd="${BA_VENV}/bin/python3"
  _init_gen_material="${BA_TARGET}/scripts/colors/generate_colors_material.py"
  ...
  if [[ -n "$_init_py" && -f "$_init_gen_material" ]]; then
    ...
    if "$_init_py" "$_init_gen_material" \
        ...
        --termscheme "${BA_TARGET}/scripts/colors/terminal/scheme-base.json" --blend_bg_fg \
      ...
      if [[ -s "$_init_scss_file" && -f "${BA_TARGET}/scripts/colors/applycolor.sh" ]]; then
        bash "${BA_TARGET}/scripts/colors/applycolor.sh" 2>/dev/null || true
        log_success "GTK/KDE/terminal theme colors applied"
      elif [[ -f "${BA_TARGET}/scripts/colors/apply-gtk-theme.sh" ]]; then
        bash "${BA_TARGET}/scripts/colors/apply-gtk-theme.sh" 2>/dev/null || true
...
if [[ ! -f "${DARKLY_COLORS_FILE}" ]]; then
  ...
  if [[ -f "${MATERIAL_COLORS}" && -f "${BA_TARGET}/scripts/colors/apply-gtk-theme.sh" ]]; then
...
#####################################################################################
if [[ -n "${BA_TARGET}" && -d "${BA_TARGET}" ]]; then
  REPO_VERSION=""
  REPO_COMMIT=""
  ...
  write_version_info_json "${BA_TARGET}/version.json" "${REPO_VERSION:-0.0.0}" "${REPO_COMMIT:-unknown}" "setup-install"
...
if ! ${quiet:-false}; then
  ...
  for _crit_file in "shell.qml" "GlobalStates.qml" "modules/common/Config.qml" \
    ...
    if [[ -f "${BA_TARGET:-${XDG_CONFIG_HOME}/quickshell/ba}/${_crit_file}" ]]; then
  ...
  for _cfg_path \
    in "${XDG_CONFIG_HOME}/niri/config.kdl:Niri config" \
       "${DOTS_CORE_CONFDIR}/config.json:BlueArchive config" \
...
if ${quiet:-false}; then
  if [[ "${IS_UPDATE}" == "true" ]]; then
    echo "BlueArchive: update complete"
  else
    echo "BlueArchive: install complete"
  fi
else
  echo ""
  echo ""
...
EOF
    ...
    echo -e "${STY_BLUE}│${STY_RST}  ${STY_GREEN}✓${STY_RST} Quickshell ba synced to ~/.config/quickshell/ba/"
...
EOF
    ...
    echo -e "${STY_BLUE}│${STY_RST}"
    echo -e "${STY_BLUE}│${STY_RST}  ${STY_GREEN}✓${STY_RST} Quickshell ba copied to ~/.config/quickshell/ba/"
    echo -e "${STY_BLUE}│${STY_RST}  ${STY_GREEN}✓${STY_RST} Niri config wired to the ba launcher"
...
if ! ${quiet:-false}; then
  ...
  echo -e "${STY_FAINT}Logs: ba logs${STY_RST}"
  echo ""

  if [[ "${IS_UPDATE}" == "true" ]]; then
    echo -e "${STY_GREEN}Done. Hot reload should kick in any second now.${STY_RST}"
  else
    echo -e "${STY_GREEN}Install complete. BlueArchive is ready through the ba launcher.${STY_RST}"
  fi
  echo ""
