# Uninstall command for BlueArchive
# Safely removes BlueArchive while preserving user data and shared resources
# This script is meant to be sourced.

# shellcheck shell=bash

BA_CONFIG_DIR="${DOTS_CORE_CONFDIR:-${XDG_CONFIG_HOME}/baOS}"

###############################################################################
# Configuration - What BlueArchive installs/manages
###############################################################################

# Categories of files:
# - ba_only: Files created exclusively by BlueArchive, safe to remove
# - shared: Files that may be used by other apps, ask before removing
# - user_data: User's personal data, preserve by default

# BlueArchive-exclusive files (safe to remove)
declare -A BA_ONLY_PATHS=(
    ["${XDG_CONFIG_HOME}/quickshell/ba"]="BlueArchive shell configuration"
    ["${BA_CONFIG_DIR}"]="BlueArchive user preferences"
    ["${XDG_STATE_HOME}/quickshell/user"]="BlueArchive state (notifications, todo)"
    ["${XDG_CACHE_HOME}/quickshell/ba"]="BlueArchive cache"
    ["${XDG_BIN_HOME}/ba"]="BlueArchive launcher"
    ["${HOME}/.local/bin/ba_super_overview_daemon.py"]="BlueArchive super daemon"
    ["${XDG_CONFIG_HOME}/systemd/user/ba.service"]="BlueArchive user service"
    ["${XDG_CONFIG_HOME}/systemd/user/ba-super-overview.service"]="BlueArchive daemon service"
    ["${XDG_CONFIG_HOME}/vesktop/themes/system24.theme.css"]="BlueArchive Vesktop theme"
    ["${XDG_CONFIG_HOME}/vesktop/themes/ba-colors.css"]="BlueArchive Vesktop colors"
    ["${XDG_CONFIG_HOME}/Vesktop/themes/system24.theme.css"]="BlueArchive Vesktop theme (alt)"
    ["${XDG_CONFIG_HOME}/Vesktop/themes/ba-colors.css"]="BlueArchive Vesktop colors (alt)"
    ["${XDG_DATA_HOME}/applications/ba.desktop"]="BlueArchive desktop entry"
    ["${XDG_DATA_HOME}/applications/ba-settings.desktop"]="BlueArchive settings desktop entry"
    ["${XDG_DATA_HOME}/icons/hicolor/scalable/apps/ba.svg"]="BlueArchive launcher icon"
    ["${HOME}/.local/bin/sync-pixel-sddm.py"]="BlueArchive SDDM theme sync helper"
)

# Shared configs - may be used by other apps or user customizations
# Format: path -> "description|app_command|critical_level"
# critical_level: essential (user likely needs), optional (can remove), ba_default (BlueArchive created)
declare -A SHARED_PATHS=(
    ["${XDG_CONFIG_HOME}/niri/config.kdl"]="Niri compositor config|niri|essential"
    ["${XDG_CONFIG_HOME}/matugen"]="BlueArchive theming templates|python3|optional"
    ...
    ["${XDG_DATA_HOME:-$HOME/.local/share}/color-schemes/Darkly.colors"]="Darkly color scheme||ba_default"
)
...
# Packages that BlueArchive may have installed - with usage context
# Format: command -> "package_name|description|shared_usage"
# shared_usage: ba_only, compositor, system_tool, optional_tool
declare -A BA_PACKAGES=(
    ["qs"]="quickshell|Shell framework|ba_only"
...
has_other_quickshell_configs() {
    ...
    if [[ -d "$qs_dir" ]]; then
        ...
        other_configs=$(find "$qs_dir" -mindepth 2 -maxdepth 2 -type f -name "shell.qml" \
            ! -path "${qs_dir}/ba/shell.qml" \
            ! -path "${qs_dir}/ba/shell.qml" 2>/dev/null | wc -l)
        [[ "$other_configs" -gt 0 ]]
    else
        return 1
    fi
...
# Check if niri config has BlueArchive-specific content or is user-customized
niri_config_is_ba_default() {
    local config="${XDG_CONFIG_HOME}/niri/config.kdl"
    [[ -f "$config" ]] && grep -qE 'spawn-at-startup "([^"]*/)?ba" "start"|spawn-at-startup "qs" "-c" "ba"|quickshell:baBackdrop' "$config" 2>/dev/null
}

# Check if niri config has user customizations beyond BlueArchive defaults
niri_config_has_user_customizations() {
    ...
    # Check if file was modified after BlueArchive install
    local install_marker="${BA_CONFIG_DIR}/installed_true"
...
get_package_removal_safety() {
    ...
    case "$pkg_usage" in
        ba_only)
            # Only used by BlueArchive - safe to remove if no other qs configs
...
uninstall_stop_services() {
    tui_info "Stopping BlueArchive services..."

    # Stop quickshell ba config
    local runtime_target="${XDG_CONFIG_HOME:-$HOME/.config}/quickshell/ba"
    qs -p "$runtime_target" kill 2>/dev/null || true

    # Stop super daemon if running
    if command -v systemctl &>/dev/null && [[ -d /run/systemd/system ]]; then
        # Stop the service
        systemctl --user stop ba.service 2>/dev/null || true
        ...
        for _wd in "$_sd_user"/*.wants; do
            [[ -d "$_wd" ]] || continue
            rm -f "$_wd/ba.service" 2>/dev/null || true
        done
        # Also try legacy disable in case old [Install] symlinks exist
        systemctl --user disable ba.service 2>/dev/null || true
        systemctl --user reset-failed ba.service 2>/dev/null || true

        if systemctl --user is-active ba-super-overview.service &>/dev/null; then
            systemctl --user disable --now ba-super-overview.service 2>/dev/null || true
        fi
        systemctl --user reset-failed ba-super-overview.service 2>/dev/null || true
    fi
...
uninstall_create_backup() {
    local backup_dir="${HOME}/.local/share/ba-uninstall-backup-$(date +%Y%m%d-%H%M%S)"

    tui_info "Creating backup before uninstall..." >&2
    mkdir -p "$backup_dir"

    # Backup BlueArchive-specific files
    if [[ -d "${XDG_CONFIG_HOME}/quickshell/ba" ]]; then
        cp -r "${XDG_CONFIG_HOME}/quickshell/ba" "$backup_dir/quickshell-ba"
    fi

    if [[ -d "${BA_CONFIG_DIR}" ]]; then
        cp -r "${BA_CONFIG_DIR}" "$backup_dir/$(basename "$BA_CONFIG_DIR")"
    fi
...
uninstall_remove_ba_only() {
    local removed=0
    local _start=$SECONDS

    tui_info "Removing BlueArchive-exclusive files..."
    echo ""

    # Pre-count existing items for N/M progress display
    local total=0
    for path in "${!BA_ONLY_PATHS[@]}"; do
        local ep
        ep=$(eval echo "$path")
        [[ -e "$ep" ]] && ((total++))
    done
    ...
    tui_success "Removed $removed BlueArchive-exclusive item(s)  (${_elapsed}s)"
}
        ...
        case "$level" in
            ...
            optional)
                if $app_installed; then
                    ...
                    rec="remove"
                    reason="app not found, BlueArchive default"
                fi
                ;;
            ba_default)
                rec="remove"
                reason="created by BlueArchive"
                ;;
        esac
...
uninstall_show_manual_steps() {
    ...
    echo ""
    echo -e "  ${STY_YELLOW}•${STY_RST} SDDM theme: /usr/share/sddm/themes/ba-pixel"
    echo -e "    ${STY_FAINT}Used by: SDDM login screen${STY_RST}"
    echo ""
    echo -e "  ${STY_YELLOW}•${STY_RST} SDDM theme drop-in: /etc/sddm.conf.d/99-ba-theme.conf"
    echo -e "    ${STY_FAINT}Used by: sets Current=ba-pixel (legacy path: /etc/sddm.conf.d/ba-theme.conf)${STY_RST}"
    echo ""
    ...
    if $ask && tui_confirm "Show commands to revert these changes?" "no"; then
        ...
        echo -e "  sudo rm -rf /usr/share/sddm/themes/ba-pixel"
        echo ""
        echo -e "  ${STY_CYAN}# Remove SDDM theme config drop-in${STY_RST}"
        echo -e "  sudo rm -f /etc/sddm.conf.d/99-ba-theme.conf /etc/sddm.conf.d/ba-theme.conf"
        echo ""
    fi
}
...
uninstall_show_packages() {
    ...
    echo ""
    echo -e "${STY_FAINT}BlueArchive may have installed these packages. Review before removing.${STY_RST}"
    echo ""
    ...
    for cmd in "${!BA_PACKAGES[@]}"; do
        ...
        case "$safety" in
            safe)
                ...
                echo -e "    ${STY_FAINT}Safe to remove (only used by BlueArchive)${STY_RST}"
                ;;
            ask)
...
run_uninstall() {
    ...
    local package_name
    echo ""
    tui_title "BlueArchive Uninstaller"
    echo ""
    ...
    # Check if installed
    if [[ ! -f "${BA_CONFIG_DIR}/installed_true" ]] && \
       [[ ! -d "${XDG_CONFIG_HOME}/quickshell/ba" ]] && \
       [[ ! -f "${BA_CONFIG_DIR}/version.json" ]] && \
       [[ ! -f "${XDG_BIN_HOME}/ba" ]]; then
        tui_warn "BlueArchive does not appear to be installed"
        return 1
    fi
    ...
    echo ""
    echo "This will remove BlueArchive from your system."
    echo ""
    ...
    echo "  • BlueArchive shell configuration will be removed"
    if is_running_niri_session; then
    ...
    # Remove BlueArchive-exclusive files (always safe)
    uninstall_remove_ba_only
    uninstall_reload_user_systemd
...
EOF
    ...
    echo -e "  cp -r $backup_dir/quickshell-ba ${XDG_CONFIG_HOME}/quickshell/ba"
    echo ""
    local _total_elapsed=$(( SECONDS - _uninstall_start ))
    echo -e "${STY_FAINT}To reinstall BlueArchive:${STY_RST}"
    echo -e "  git clone https://github.com/RhythmGC/dotfiles-hyprland.git && cd dotfiles-hyprland/.config/quickshell/ba && ./setup install"
    echo ""
...
run_uninstall_quick() {
    echo ""
    tui_title "BlueArchive Quick Uninstall"
    echo ""

    echo -e "${STY_YELLOW}Quick uninstall will:${STY_RST}"
    echo "  • Remove BlueArchive-exclusive files only"
    ...
    # Remove only BlueArchive-exclusive files
    ask=false  # Disable prompts for shared configs
    uninstall_remove_ba_only
    uninstall_reload_user_systemd

    echo ""
    tui_success "BlueArchive removed. All shared configs and packages preserved."
    echo ""
