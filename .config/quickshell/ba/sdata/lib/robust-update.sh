# Robust update system for BlueArchive
...
# Configuration
#####################################################################################
BA_TARGET="${XDG_CONFIG_HOME}/quickshell/ba"
BA_BACKUP_DIR="${XDG_STATE_HOME}/quickshell/backups"
BA_MANIFEST_FILE="${BA_TARGET}/.ba-manifest"
...
create_update_backup() {
    ...
    local backup_path="${BA_BACKUP_DIR}/pre-update-${timestamp}"
    
    mkdir -p "$backup_path"
    
    # Only backup QML code, not user configs
    rsync -a --exclude='.ba-manifest' "$target_dir/" "$backup_path/" 2>/dev/null
    
    # Save backup path for potential rollback
    echo "$backup_path" > "${BA_BACKUP_DIR}/.last-backup"
...
rollback_update() {
    local last_backup_file="${BA_BACKUP_DIR}/.last-backup"
    ...
    # Restore from backup
    rsync -a --delete "$backup_path/" "$BA_TARGET/"
...
verify_qs_loads() {
    ...
    qs -p "$BA_TARGET" kill 2>/dev/null || true
    sleep 0.5
    ...
    output=$(timeout "$timeout_sec" qs -p "$BA_TARGET" 2>&1) || exit_code=$?
...
run_verification() {
    ...
    if [[ -d "$BA_TARGET/scripts" ]]; then
        local scripts_without_exec
        scripts_without_exec=$(find "$BA_TARGET/scripts" -name "*.sh" -o -name "*.fish" -o -name "*.py" | while read -r f; do
            [[ ! -x "$f" ]] && echo "$f"
        done)
        
        if [[ -n "$scripts_without_exec" ]]; then
            log_warning "Fixing script permissions..."
            find "$BA_TARGET/scripts" \( -name "*.sh" -o -name "*.fish" -o -name "*.py" \) -exec chmod +x {} \;
        fi
    fi
...
perform_robust_update() {
    ...
    # 4. Install manifest
    cp "$temp_manifest" "$BA_MANIFEST_FILE"
    rm -f "$temp_manifest"
    
    # 5. Cleanup orphans
    log_info "Checking for orphan files..."
    cleanup_orphans "$target_dir" "$BA_MANIFEST_FILE"
    ...
    cleanup_old_backups "$BA_BACKUP_DIR" 5
