#!/bin/bash
# Versioning system for BlueArchive
...
CONFIG_DIR="${XDG_CONFIG_HOME_RESOLVED}/baOS"

RUNTIME_DIR_USER="${XDG_CONFIG_HOME_RESOLVED}/quickshell/ba"
RUNTIME_DIR_SYSTEM_LOCAL="${BA_SYSTEM_RUNTIME_DIR_LOCAL:-/usr/local/share/quickshell/ba}"
RUNTIME_DIR_SYSTEM="${BA_SYSTEM_RUNTIME_DIR:-/usr/share/quickshell/ba}"
LEGACY_RUNTIME_DIR_USER="${XDG_CONFIG_HOME_RESOLVED}/quickshell/ba"
LEGACY_RUNTIME_DIR_SYSTEM_LOCAL="${BA_LEGACY_SYSTEM_RUNTIME_DIR_LOCAL:-/usr/local/share/quickshell/ba}"
LEGACY_RUNTIME_DIR_SYSTEM="${BA_LEGACY_SYSTEM_RUNTIME_DIR:-/usr/share/quickshell/ba}"
...
GITHUB_REPO="RhythmGC/dotfiles-hyprland"
GITHUB_API="https://api.github.com/repos/${GITHUB_REPO}"

# Cache for remote version checks (avoid hammering GitHub)
VERSION_CACHE_FILE="${XDG_CACHE_HOME:-$HOME/.cache}/ba/version-cache.json"
VERSION_CACHE_TTL=3600  # 1 hour in seconds

get_runtime_shell_dir() {
    local override="${BA_RUNTIME_DIR:-}"
...
get_repo_commit() {
    if command -v git &>/dev/null && [[ ( -e "${REPO_ROOT}/.git" || -e "${REPO_ROOT}/../../../.git" ) ]]; then
...
get_installed_version_file() {
    ...
    if [[ -n "${BA_RUNTIME_DIR:-}" && -f "$runtime_version_file" ]]; then
...
get_installed_commit() {
    ...
    if [[ "$(get_installed_install_mode)" == "repo-link" ]]; then
        local runtime_dir
        runtime_dir="$(get_runtime_shell_dir)"
        if [[ -n "$runtime_dir" && ( -e "$runtime_dir/.git" || -e "$runtime_dir/../../../.git" ) ]]; then
            git -C "$runtime_dir" rev-parse --short HEAD 2>/dev/null || echo "unknown"
            return
        elif [[ ( -e "${REPO_ROOT}/.git" || -e "${REPO_ROOT}/../../../.git" ) ]]; then
            get_repo_commit
            return
        fi
    fi
...
get_default_package_update_hint() {
    case "$(get_installed_package_manager)" in
        ...
        *) echo "use your package manager to update BlueArchive" ;;
    esac
}
...
get_install_mode() {
    if [[ -n "${BA_INSTALL_MODE:-}" ]]; then
        echo "$BA_INSTALL_MODE"
        return
    fi

    local target="${XDG_CONFIG_HOME_RESOLVED}/quickshell/ba"
    local repo_real=""
    local target_real=""
    ...
    if [[ ( -e "${REPO_ROOT:-}/.git" || -e "${REPO_ROOT:-}/../../../.git" ) && -f "${REPO_ROOT:-}/setup" && -f "${REPO_ROOT:-}/shell.qml" ]]; then
        echo "repo-copy"
        return
    fi
...
get_update_strategy() {
    if [[ -n "${BA_UPDATE_STRATEGY:-}" ]]; then
        echo "$BA_UPDATE_STRATEGY"
        return
    fi
...
get_version_repo_path() {
    if [[ -n "${BA_REPO_PATH:-}" ]]; then
        printf '%s' "$BA_REPO_PATH"
        return
    fi
...
write_version_info_json() {
    ...
    local package_manager="${BA_PACKAGE_MANAGER:-}"
    local package_name="${BA_PACKAGE_NAME:-}"
    local package_update_hint="${BA_PACKAGE_UPDATE_HINT:-}"
...
show_version_status() {
    ...
    echo ""
    echo -e "${STY_CYAN}${STY_BOLD}BlueArchive Version Status${STY_RST}"
    echo ""
...
show_changelog() {
    ...
    echo -e "${STY_CYAN}${STY_BOLD}BlueArchive Changelog${STY_RST}"
    echo ""
