# Package installation functions for BlueArchive
...
install-python-packages(){
  ...
  if [[ ! -d "$venv_dir/bin" ]]; then
    ...
    x uv venv --prompt ba-venv "$venv_dir" -p 3.12 || uv venv --prompt ba-venv "$venv_dir" || {
  ...
  if [[ ! -f "$requirements_file" ]]; then
    if declare -F get_runtime_shell_dir >/dev/null; then
      ...
      requirements_file="${XDG_CONFIG_HOME:-$HOME/.config}/quickshell/ba/sdata/uv/requirements.txt"
    fi
  fi
...
setup-environment-config(){
  ...
  if pacman -Q plasma-desktop &>/dev/null 2>&1 || pacman -Q plasma-workspace &>/dev/null 2>&1 || \
     dpkg -l plasma-desktop 2>/dev/null | grep -q '^ba' || \
     rpm -q plasma-desktop &>/dev/null 2>&1; then
    qt_theme="kde"
  fi

  mkdir -p ~/.config/environment.d
  cat > ~/.config/environment.d/ba.conf << EOF
# BlueArchive environment variables
...
BA_VENV_COMPAT=\$HOME/.local/state/quickshell/.venv
BA_VENV=\$HOME/.local/state/quickshell/.venv
EOF
...
include=~/.config/foot/ba-colors.ini
...
EOF
    ...
    if ! grep -q "include=.*ba-colors\.ini" ~/.config/foot/foot.ini; then
      log_info "Adding ba-colors.ini include to existing foot.ini..."
      sed -i '1i include=~/.config/foot/ba-colors.ini' ~/.config/foot/foot.ini
    fi
...
# BlueArchive wallpaper theming - colors from quickshell
...
if status is-interactive
    ...
    # Load terminal colors from ba theming
...
setup-bash-config(){
  ...
  local ba_config="$HOME/.config/baOS/bashrc"

  mkdir -p ~/.config/baOS

  # Create ba bash config
  cat > "$ba_config" << 'EOF'
# ba shell integration - starship prompt and terminal colors
...
EOF
  ...
  if [[ -f "$bashrc" ]]; then
    if grep -q "source.*ba/bashrc" "$bashrc" || grep -q "\..*ba/bashrc" "$bashrc"; then
      sed -i 's|~/.config/ba/bashrc|~/.config/baOS/bashrc|g' "$bashrc"
      log_success "Updated Bash integration path in .bashrc"
    elif ! grep -q "source.*ba/bashrc" "$bashrc" && ! grep -q "\..*ba/bashrc" "$bashrc"; then
      echo -e "\n# ba shell integration\n[[ -f ~/.config/baOS/bashrc ]] && source ~/.config/baOS/bashrc" >> "$bashrc"
      log_success "Added Bash integration to .bashrc"
    else
      log_info "Bash integration already in .bashrc"
    fi
  else
    echo -e "# ba shell integration\n[[ -f ~/.config/baOS/bashrc ]] && source ~/.config/baOS/bashrc" > "$bashrc"
...
setup-zsh-config(){
  ...
  local ba_config="$HOME/.config/baOS/zshrc"

  mkdir -p ~/.config/baOS

  # Create ba zsh config
  cat > "$ba_config" << 'EOF'
# ba shell integration - starship prompt and terminal colors
...
EOF
  ...
  if [[ -f "$zshrc" ]]; then
    if grep -q "source.*ba/zshrc" "$zshrc" || grep -q "\..*ba/zshrc" "$zshrc"; then
      sed -i 's|~/.config/ba/zshrc|~/.config/baOS/zshrc|g' "$zshrc"
      log_success "Updated Zsh integration path in .zshrc"
    elif ! grep -q "source.*ba/zshrc" "$zshrc" && ! grep -q "\..*ba/zshrc" "$zshrc"; then
      echo -e "\n# ba shell integration\n[[ -f ~/.config/baOS/zshrc ]] && source ~/.config/baOS/zshrc" >> "$zshrc"
