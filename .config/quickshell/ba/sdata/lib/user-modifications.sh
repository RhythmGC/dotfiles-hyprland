# User modification detection for BlueArchive
...
manifest_has_checksums() {
    ...
    # v2 manifests may have either the legacy ba header or the current ba header
    head -1 "$manifest_file" | grep -qE "(ba|ba)-manifest v2" && return 0
...
find_runtime_manifest_file() {
    local target_dir="$1"
    local manifest
    for manifest in "${target_dir}/.ba-manifest" "${target_dir}/.ba-manifest"; do
        if [[ -f "$manifest" ]]; then
            printf '%s\n' "$manifest"
            return 0
        fi
...
show_file_diff() {
    ...
    local original="${snapshot_dir}/ba/${file_path}"
    if [[ ! -f "$original" ]]; then
        original="${snapshot_dir}/ba/${file_path}"
    fi
    ...
    while true; do
        ...
        case "$choice" in
            "Preserve & Continue")
                ...
                tui_dim "    cp $PRESERVED_MODS_DIR/<path> ~/.config/quickshell/ba/<path>"
                return 0
                ;;
            "View Changes")
                ...
                if [[ -n "$latest_snapshot" ]] && [[ -d "${SNAPSHOTS_DIR}/${latest_snapshot}/ba" ]]; then
                    echo ""
                    local shown=0
                ...
                elif [[ -n "$latest_snapshot" ]] && [[ -d "${SNAPSHOTS_DIR}/${latest_snapshot}/ba" ]]; then
                    echo ""
                    local shown=0
                    while IFS= read -r f && [[ $shown -lt 3 ]]; do
                        [[ -z "$f" ]] && continue
