# Doctor command for BlueArchive
...
check_repo_checkout_state() {
    ...
    if [[ ! ( -e "${REPO_ROOT}/.git" || -e "${REPO_ROOT}/../../../.git" ) ]]; then
        doctor_fail "Repo checkout is missing git metadata"
        echo -e "    ${STY_FAINT}Run setup from a real BlueArchive checkout, not a random copy${STY_RST}"
        return 1
    fi
...
check_launcher_health() {
    ...
    launcher_cmd="$(command -v ba 2>/dev/null || true)"
    expected_launcher="${XDG_BIN_HOME}/ba"
    repo_launcher="${REPO_ROOT}/scripts/ba"
    runtime_launcher="$(doctor_runtime_dir 2>/dev/null)/scripts/ba"

    if [[ "$installed_strategy" == "package-manager" ]]; then
        if [[ -n "$launcher_cmd" ]]; then
            doctor_pass "Launcher available"
        else
            doctor_fail "ba launcher not found in PATH"
...
check_version_tracking() {
    ...
    local legacy_version_file="${XDG_CONFIG_HOME}/baOS/version.json"
...
check_manifest() {
    ...
    local manifest="${target}/.ba-manifest"
    local installed_marker="${DOTS_CORE_CONFDIR}/installed_true"
    local installed_strategy
    installed_strategy=$(get_installed_update_strategy)
...
check_service_unit_health() {
    ...
    installed_strategy="$(get_installed_update_strategy)"
    service_path="${XDG_CONFIG_HOME}/systemd/user/ba.service"
    ...
    if [[ ! -f "$service_path" ]]; then
        if [[ "$installed_strategy" == "package-manager" ]]; then
            ...
            doctor_fail "User ba.service missing"
            echo -e "    ${STY_FAINT}Run: ba service install${STY_RST}"
        fi
        return 0
    fi

    if [[ "$installed_strategy" != "package-manager" ]] && declare -F sync_user_ba_service_from_repo_if_present >/dev/null 2>&1; then
        if sync_user_ba_service_from_repo_if_present >/dev/null 2>&1; then
            doctor_fix "Refreshed user ba.service from repo"
        fi
    fi

    local kill_mode fragment_path
    kill_mode="$(systemctl --user show -p KillMode ba.service 2>/dev/null | cut -d= -f2)"
    fragment_path="$(systemctl --user show -p FragmentPath ba.service 2>/dev/null | cut -d= -f2)"

    if [[ -n "$kill_mode" && "$kill_mode" != "process" ]]; then
        doctor_fail "ba.service KillMode is '${kill_mode}'"
        echo -e "    ${STY_FAINT}Run: ba service install${STY_RST}"
    else
    ...
    if [[ -n "$fragment_path" && "$fragment_path" != "$service_path" ]]; then
        tui_warn "systemd is loading ba.service from ${fragment_path}"
    fi
    ...
    for wants_dir in "${XDG_CONFIG_HOME}/systemd/user"/*.wants; do
        ...
        [[ -e "$wants_dir/ba.service" || -L "$wants_dir/ba.service" ]] || continue
        local target_name
    ...
    if [[ -n "$expected_target" && "$has_expected_link" == false ]]; then
        if [[ "$installed_strategy" != "package-manager" ]] && declare -F ensure_user_ba_service_enabled >/dev/null 2>&1 && ensure_user_ba_service_enabled >/dev/null 2>&1; then
            doctor_fix "Enabled ba.service for ${expected_target}"
        else
            doctor_fail "ba.service not wired to ${expected_target}"
            echo -e "    ${STY_FAINT}Run: ba service enable${STY_RST}"
        fi
    fi

    if [[ ${#stale_links[@]} -gt 0 ]]; then
        doctor_fail "Stale service links found: ${stale_links[*]}"
        echo -e "    ${STY_FAINT}Run: ba service disable && ba service enable${STY_RST}"
    fi
...
check_quickshell_abi() {
    ...
    # See: https://github.com/RhythmGC/dotfiles-hyprland/issues
    ...
    if [[ -n "$_rebuild_cmd" ]]; then
        ...
        echo -e "  ${STY_FAINT}Or run: ba doctor --fix-abi${STY_RST}"
    else
...
check_quickshell_loads() {
    ...
    if ! kill -0 "$qs_pid" 2>/dev/null; then
        ...
        if echo "$output" | grep -qiE "built against Qt|Qt.*mismatch|incompatible Qt"; then
            ...
            echo -e "  ${STY_YELLOW}Run: ba doctor  (to auto-rebuild quickshell)${STY_RST}"
            return 1
        fi
...
check_conflicting_services() {
    ...
    if [[ ${#running[@]} -gt 0 ]]; then
        ...
        doctor_fix "Stopped conflicting: ${running[*]} (BlueArchive has built-in notifications, re-enable with: systemctl --user enable <service>)"
    else
...
check_conflicting_shells() {
    # Quickshell-based shells that conflict with BlueArchive at the package level.
    ...
    if [[ ${#found[@]} -gt 0 ]]; then
        ...
        echo -e "    ${STY_FAINT}These must be removed for BlueArchive to work. Run: ./setup install${STY_RST}"
    else
...
check_environment_vars() {
    ...
    # Check bash — look for BA_VENV (canonical) or BA_VENV_COMPAT (legacy)
    if [[ -f "$HOME/.bashrc" ]] && ! grep -q "BA_VENV" "$HOME/.bashrc" 2>/dev/null; then
        cat >> "$HOME/.bashrc" << BEOF

# BlueArchive environment
export BA_VENV="${venv_path}"
export BA_VENV_COMPAT="\$BA_VENV"
# end BlueArchive
BEOF
        ((fixed++)) || true
    fi
    ...
    # Check fish
    local fish_conf="${XDG_CONFIG_HOME}/fish/conf.d/ba-env.fish"
    if command -v fish &>/dev/null && [[ ! -f "$fish_conf" ]]; then
        mkdir -p "$(dirname "$fish_conf")"
        cat > "$fish_conf" << FEOF
# BlueArchive environment — auto-generated by doctor
set -gx BA_VENV "${venv_path}"
set -gx BA_VENV_COMPAT "\$BA_VENV"
FEOF
        ((fixed++)) || true
    fi
    
    # Check zsh
    if [[ -f "$HOME/.zshrc" ]] && ! grep -q "BA_VENV" "$HOME/.zshrc" 2>/dev/null; then
        cat >> "$HOME/.zshrc" << ZEOF
...
run_doctor_with_fixes() {
            ...
            if tui_confirm "Rebuild quickshell to fix the ABI mismatch?"; then
                ...
                if $_sudo_ok; then
                    ...
                    if [[ $_rebuild_rc -eq 124 ]]; then
                        ...
                        rm -f "${XDG_CACHE_HOME:-$HOME/.cache}/ba/abi-check" 2>/dev/null
                        if _doctor_abi_detect; then
                            echo ""
    ...
    if [[ $doctor_failed -gt 0 ]]; then
        ...
        tui_info "Then read logs: ba logs"
        return 1
    elif [[ $doctor_fixed -gt 0 ]]; then
        tui_success "All issues fixed automatically."
        tui_info "Restart the shell to apply: ba restart"
    else
