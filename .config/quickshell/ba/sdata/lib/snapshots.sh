# Snapshot system for BlueArchive
...
# Paths to snapshot
SNAPSHOT_PATHS=(
    "${XDG_CONFIG_HOME}/quickshell/ba"
    "${BA_CONFIG_DIR}/config.json"
...
create_snapshot() {
    ...
    if [[ -d "${XDG_CONFIG_HOME}/quickshell/ba" ]]; then
        rsync -a --exclude='.ba-manifest' "${XDG_CONFIG_HOME}/quickshell/ba/" "${snapshot_dir}/ba/"
    fi
    ...
    # Copy user config
    if [[ -f "${BA_CONFIG_DIR}/config.json" ]]; then
        cp "${BA_CONFIG_DIR}/config.json" "${snapshot_dir}/"
    fi
    ...
    if [[ -f "${BA_CONFIG_DIR}/migrations.json" ]]; then
        cp "${BA_CONFIG_DIR}/migrations.json" "${snapshot_dir}/"
    fi
...
restore_snapshot() {
    ...
    # Stop shell
    local runtime_target="${XDG_CONFIG_HOME}/quickshell/ba"
    qs -p "$runtime_target" kill &>/dev/null || true
    
    # Restore QML code
    if [[ -d "${snapshot_dir}/ba" ]]; then
        log_info "Restoring QML code..."
        rsync -a --delete "${snapshot_dir}/ba/" "${XDG_CONFIG_HOME}/quickshell/ba/"
    elif [[ -d "${snapshot_dir}/ba" ]]; then
        log_info "Restoring QML code..."
        rsync -a --delete "${snapshot_dir}/ba/" "${XDG_CONFIG_HOME}/quickshell/ba/"
    fi
    ...
    if [[ -f "${snapshot_dir}/config.json" ]]; then
        ...
        mkdir -p "${BA_CONFIG_DIR}"
        cp "${snapshot_dir}/config.json" "${BA_CONFIG_DIR}/"
    fi
    ...
    if [[ -f "${snapshot_dir}/migrations.json" ]]; then
        mkdir -p "${BA_CONFIG_DIR}"
        cp "${snapshot_dir}/migrations.json" "${BA_CONFIG_DIR}/"
    fi
    ...
    if [[ -n "$NIRI_SOCKET" ]] || [[ -n "$WAYLAND_DISPLAY" ]]; then
        ...
        tui_info "Run: ba start (in your Niri session)"
