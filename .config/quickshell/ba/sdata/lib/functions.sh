# Core functions for BlueArchive installer
...
cp_file(){
  ...
  # (e.g. when ~/.config/quickshell/ba points into the repo).
  if [[ -e "$dst" ]]; then
...
function ensure_launcher_path_in_shells(){
  ...
  local marker="# BlueArchive launcher PATH"
  local end_marker="# end BlueArchive launcher PATH"
  local sh_block="
${marker}
case \":\$PATH:\" in
  *:\"${launcher_dir}\":*) ;;
...
"
  ...
  cat > "${fish_conf_dir}/ba-path.fish" << EOF
