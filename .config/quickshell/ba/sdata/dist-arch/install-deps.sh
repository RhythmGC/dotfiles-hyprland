# Install dependencies for BlueArchive on Arch-based systems
...
for pkgdir in ./sdata/dist-arch/ba-*/; do
  # Check group flags
  pkgname=$(basename "$pkgdir")
  case "$pkgname" in
    ba-audio) $INSTALL_AUDIO || continue ;;
    ba-toolkit) $INSTALL_TOOLKIT || continue ;;
    ba-screencapture) $INSTALL_SCREENCAPTURE || continue ;;
    ba-fonts) $INSTALL_FONTS || continue ;;
  esac
...
for _qs_pkg in "${_qs_shell_conflicts[@]}"; do
  if pacman -Qi "$_qs_pkg" &>/dev/null 2>&1; then
    ...
    log_warning "$_qs_pkg is installed and conflicts with BlueArchive"
    ...
    if $ask; then
      if tui_confirm "Remove $_qs_pkg? (required for BlueArchive)"; then
        ...
        log_warning "Keeping $_qs_pkg — BlueArchive may not work correctly"
      fi
    else
...
if $INSTALL_AUDIO; then
  : # cava moved to ba-audio PKGBUILD
fi
...
# This prevents "clean orphans" from removing BlueArchive's deps.
# The meta-package contains no files — only dependency declarations.
#####################################################################################
tui_info "Registering dependencies with pacman..."

_meta_dir="./sdata/dist-arch/ba-deps"
if [[ -f "$_meta_dir/PKGBUILD" ]]; then
  # Update pkgver from VERSION file
  _ba_ver="$(cat ./VERSION 2>/dev/null || echo '2.27.0')"
  sed -i "s/^pkgver=.*/pkgver=${_ba_ver}/" "$_meta_dir/PKGBUILD"
    ...
    if makepkg -dfC 2>/dev/null; then
      ...
      if [[ -f "${local_pkg[0]}" ]]; then
        if pkg_sudo pacman -U --noconfirm --needed "${local_pkg[0]}" 2>/dev/null; then
          log_success "Meta-package ba-deps registered — orphan cleaner will skip BlueArchive deps"
        else
          ...
          pkg_sudo pacman -Udd --noconfirm "${local_pkg[0]}" 2>/dev/null && \
            log_success "Meta-package ba-deps registered (forced)" || \
