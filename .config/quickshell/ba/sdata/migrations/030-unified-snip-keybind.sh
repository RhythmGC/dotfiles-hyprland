...
migration_check() {
  ...
  # Apply only if this is an BlueArchive-managed binds file (has the region binds),
...
migration_preview() {
  ...
  echo -e "${STY_GREEN}+ Ctrl+Shift+S { spawn \"ba\" \"region\" \"menu\"; }${STY_RST}"
}
...
migration_apply() {
  ...
  if grep -q 'region" "search"' "$binds_file"; then
    sed -i '/region" "search";/a\    Ctrl+Shift+S { spawn "ba" "region" "menu"; }' "$binds_file"
  else
    sed -i '/region" "screenshot";/a\    Ctrl+Shift+S { spawn "ba" "region" "menu"; }' "$binds_file"
  fi
}
