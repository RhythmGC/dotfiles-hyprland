...
MIGRATION_DESCRIPTION="Pre-2.26 versions of BlueArchive shipped /etc/sddm.conf.d/ba-theme.conf and worked around the alphabetical merge order by silently sed -i'ing InputMethod=qtvirtualkeyboard out of foreign drop-ins like kde_settings.conf. That stomped on user/KDE settings every install/update. The new install uses /etc/sddm.conf.d/99-ba-theme.conf which sorts last alphabetically and wins by drop-in order, so BlueArchive no longer needs to touch foreign files. This migration renames the legacy drop-in for existing users."
MIGRATION_TARGET_FILE="/etc/sddm.conf.d/ba-theme.conf"
MIGRATION_REQUIRED=true

_sddm_legacy_path="/etc/sddm.conf.d/ba-theme.conf"
_sddm_new_path="/etc/sddm.conf.d/99-ba-theme.conf"
...
migration_preview() {
  ...
  echo -e "${STY_FAINT}  The new name sorts after kde_settings.conf etc. so BlueArchive's settings"
