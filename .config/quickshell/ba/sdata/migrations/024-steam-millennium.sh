...
MIGRATION_TARGET_FILE="~/.config/baOS/config.json"
MIGRATION_REQUIRED=true

_steam_migration_config_file() {
  local xdg_config_home="${XDG_CONFIG_HOME:-$HOME/.config}"
  if [[ -L "$xdg_config_home/baOS" && -f "$xdg_config_home/ba/config.json" ]]; then
    printf '%s\n' "$xdg_config_home/ba/config.json"
  elif [[ -d "$xdg_config_home/baOS" || -f "$xdg_config_home/baOS/config.json" ]]; then
    printf '%s\n' "$xdg_config_home/baOS/config.json"
  elif [[ -f "$xdg_config_home/ba/config.json" ]]; then
    printf '%s\n' "$xdg_config_home/ba/config.json"
  else
    printf '%s\n' "$xdg_config_home/ba/config.json"
  fi
...
PY
  ...
  if _steam_migration_runtime_available && _steam_migration_has_steam_dir && [[ -x "$repo_root/scripts/colors/modules/70-steam.sh" ]]; then
    BA_STEAM_THEME_FORCE=1 bash "$repo_root/scripts/colors/modules/70-steam.sh" >/dev/null 2>&1 || true
  fi
