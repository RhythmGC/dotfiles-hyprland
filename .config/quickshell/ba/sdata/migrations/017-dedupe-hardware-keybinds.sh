...
migration_check() {
  ...
  # Match both bare "ba" and full-path "/home/user/.local/bin/ba" patterns,
  # since migration 016 rewrites all binds to the full launcher path.
  local brightness_count media_count
  brightness_count="$(grep -cE 'XF86MonBrightnessUp \{ spawn "[^"]*ba" "brightness" "increment"; \}' "$config" || true)"
  media_count="$(grep -cE 'XF86AudioPlay \{ spawn "[^"]*ba" "mpris" "playPause"; \}' "$config" || true)"
...
# Match blocks with any ba path variant (bare "ba" or full "/path/to/ba")
BA_RE = re.escape(launcher_path) + r'|ba'
...
def matches_block(lines, start, comment, keys, ba_re):
    ...
    for idx, (key, target, func) in enumerate(keys):
        pattern = rf'^{key}\s+\{{\s+spawn\s+"(?:{ba_re})"\s+"{target}"\s+"{func}";\s+\}}$'
...
while i < len(lines):
    ...
    if matches_block(lines, i, BRIGHTNESS_COMMENT, BRIGHTNESS_KEYS, BA_RE):
        if seen_brightness:
            i += block_len_bright
            continue
    ...
    if matches_block(lines, i, MEDIA_COMMENT, MEDIA_KEYS, BA_RE):
        if seen_media:
            i += block_len_media
            continue
        seen_media = True
