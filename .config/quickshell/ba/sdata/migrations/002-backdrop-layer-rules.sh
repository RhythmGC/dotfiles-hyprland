...
migration_check() {
  ...
  [[ -f "$config" ]] && ! grep -q "quickshell:baBackdrop" "$config"
}

migration_preview() {
  echo -e "${STY_GREEN}+ layer-rule { match namespace=\"quickshell:baBackdrop\" ... }${STY_RST}"
...
layer-rule {
    match namespace="quickshell:baBackdrop"
...
// ============================================================================
// Layer rules added by BlueArchive (required for backdrop in overview)
// ============================================================================
layer-rule {
    match namespace="quickshell:baBackdrop"
    place-within-backdrop true
