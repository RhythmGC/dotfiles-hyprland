...
migration_apply() {
  ...
  export BA_MIGRATION_NIRI_CONFIG="$config"
  export BA_MIGRATION_LAUNCHER_PATH="${XDG_BIN_HOME:-$HOME/.local/bin}/ba"
  python3 << 'MIGRATE'
import os
import shutil
import time
...
config_path = Path(os.environ["BA_MIGRATION_NIRI_CONFIG"]).expanduser()
launcher_path = os.environ["BA_MIGRATION_LAUNCHER_PATH"]
...
binds_text = normalize_text(buckets["70"]).replace(
    ...
    'spawn "bash" "-lc" "exec \\"$(ba path)/scripts/launch-terminal.sh\\""',
    f'spawn "{launcher_path}" "terminal"',
).replace(
    'spawn "bash" "-lc" "exec \\"$(ba path)/scripts/close-window.sh\\""',
...
// ─────────────────────────────────────────────────────────────────────────────
//
// This file is YOURS. BlueArchive updates will never overwrite it.
