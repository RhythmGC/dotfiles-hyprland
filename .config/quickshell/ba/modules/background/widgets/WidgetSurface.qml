...
// Adapts to the active ba style: blur for aurora/angel, border-only for ba, solid for material.
...
Rectangle {
    ...
    readonly property bool _aurora: Appearance.auroraEverywhere && !Appearance.baEverywhere
    readonly property bool _ba: Appearance.baEverywhere
    ...
    Rectangle {
        ...
        border.color: root._ba
            ? ColorUtils.applyAlpha(Appearance.ba.colBorder, root.surfaceBorderOpacity * 3)
    ...
    Rectangle {
        anchors.fill: parent
        visible: root._ba && root.surfaceOpacity > 0
        radius: root.radius
        color: ColorUtils.applyAlpha(Appearance.ba.colLayer1, root.surfaceOpacity * 2)
    }
}
