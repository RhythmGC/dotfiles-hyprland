...
WSettingsPage {
    ...
    WSettingsCard {
        ...
        ColumnLayout {
            ...
            WText {
                Layout.fillWidth: true
                text: Translation.tr("Based on baOS by end-4, adapted for the Niri compositor.")
    ...
    WSettingsCard {
        ...
        WSettingsRow {
            ...
            description: FileUtils.trimFileProtocol(`${Directories.config}/baOS/`)
            icon: "folder"
        }
        
        WSettingsRow {
            label: Translation.tr("Shell path")
            description: FileUtils.trimFileProtocol(`${Directories.config}/quickshell/ba/`)
            icon: "folder"
        }
        
        WSettingsRow {
            label: Translation.tr("Panel family")
            description: Config.options?.panelFamily === "waffle" ? "Waffle (Windows 11)" : "ba (Material)"
            icon: "app-generic"
        }
    }
}
