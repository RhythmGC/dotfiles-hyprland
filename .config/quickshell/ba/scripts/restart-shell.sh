...
#
# Delegates to `ba restart` which correctly kills the wrapper process
...
launcher_script="$config_dir/scripts/ba"

if [[ ! -f "$launcher_script" ]]; then
    launcher_script="$script_dir/ba"
fi

if [[ ! -f "$launcher_script" ]]; then
    echo "restart-shell: cannot find ba launcher" >&2
    exit 1
fi
