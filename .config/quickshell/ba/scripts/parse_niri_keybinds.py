#!/usr/bin/env python3
"""
Parse niri config.kdl to extract keybinds for ba cheatsheet.
...
def parse_ba_action(action: str) -> tuple[str, str] | None:
    ipc_match = re.search(r'spawn\s+"(?:[^"]*/)?ba"\s+"ipc"\s+"call"\s+"([\w-]+)"\s+"([\w-]+)"', action)
    if ipc_match:
        return ipc_match.group(1), ipc_match.group(2)

    settings_match = re.search(r'spawn\s+"(?:[^"]*/)?ba"\s+"settings"(?:\s|;|$)', action)
    if settings_match:
        return 'settings', 'open'

    terminal_match = re.search(r'spawn\s+"(?:[^"]*/)?ba"\s+"terminal"(?:\s|;|$)', action)
    if terminal_match:
        return 'launcher', 'terminal'

    close_window_match = re.search(r'spawn\s+"(?:[^"]*/)?ba"\s+"close-window"(?:\s|;|$)', action)
    if close_window_match:
        return 'launcher', 'close-window'

    browser_match = re.search(r'spawn\s+"(?:[^"]*/)?ba"\s+"browser"(?:\s|;|$)', action)
    if browser_match:
        return 'browser', 'open'

    direct_match = re.search(r'spawn\s+"(?:[^"]*/)?ba"\s+"([\w-]+)"\s+"([\w-]+)"', action)
    if direct_match:
...
def generate_comment(action: str) -> str:
    ...
    if action.startswith('spawn'):
        ba_action = parse_ba_action(action)
        if ba_action:
            target, func = ba_action
...
def categorize_keybind(kb: dict) -> str:
    ...
    # BlueArchive Shell
    if any(x in comment for x in ['ba ', 'clipboard', 'lock screen', 'wallpaper', 'settings', 'cheatsheet', 'panel style']):
        return 'BlueArchive Shell'
    ba_action = parse_ba_action(action)
    if ba_action:
        target, _func = ba_action
...
def parse_niri_config(config_path: Path) -> dict:
    ...
    category_order = [
        'System', 'BlueArchive Shell', 'Window Switcher', 'Screenshots',
