#!/usr/bin/env python3
"""
niri-config.py — Niri compositor configuration helper for BlueArchive settings UI.
...
Commands:
  ...
  detect-customizations Report Niri files that differ from shipped BlueArchive defaults
        ...
        add_file(
            {
                ...
                "reason": "This managed Niri file differs from the shipped BlueArchive default.",
            ...
            add_file(
                {
                    ...
                    if relative_path == "config.d/15-outputs.kdl"
                    else "Additional user config file not shipped by BlueArchive defaults.",
                    "preview": preview,
...
def _sync_cursor_env(theme=None, size=None):
    ...
    The install may place XCURSOR_THEME in ba.conf or the user might have
    ...
    target_file = None
    for candidate in ["ba.conf", "cursor.conf"]:
...
# niri-config.py is called with a full path from the ba launcher.
...
_KB_IPC_MAP = {
    ...
    ("overlay", "toggle"): "BlueArchive Overlay",
    ("overview", "toggle"): "BlueArchive Overview",
    ...
    ("coverflowSelector", "toggle"): "Wallpaper coverflow selector",
...
def _kb_parse_ba_action(action: str):
    """Detect ba IPC calls and return (target, function) or None."""
    m = re.search(
        r'spawn\s+"(?:[^"]*/)?ba"\s+"ipc"\s+"call"\s+"([\w-]+)"\s+"([\w-]+)"', action
    )
    if m:
        return m.group(1), m.group(2)
    if re.search(r'spawn\s+"(?:[^"]*/)?ba"\s+"settings"(?:\s|;|$)', action):
        return "settings", "open"
    if re.search(r'spawn\s+"(?:[^"]*/)?ba"\s+"terminal"(?:\s|;|$)', action):
        return "launcher", "terminal"
    if re.search(r'spawn\s+"(?:[^"]*/)?ba"\s+"close-window"(?:\s|;|$)', action):
        return "launcher", "close-window"
    if re.search(r'spawn\s+"(?:[^"]*/)?ba"\s+"browser"(?:\s|;|$)', action):
        return "browser", "open"
    m = re.search(r'spawn\s+"(?:[^"]*/)?ba"\s+"([\w-]+)"\s+"([\w-]+)"', action)
    if m:
        return m.group(1), m.group(2)
    return None
...
def _kb_generate_comment(action: str) -> str:
    ...
    if action.startswith("spawn"):
        ba = _kb_parse_ba_action(action)
        if ba:
            return _KB_IPC_MAP.get(ba, f"{ba[0]} {ba[1]}")
