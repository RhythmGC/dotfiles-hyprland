# Bash completion for ba CLI
# Install: eval "$(ba completions bash)"
# Or: ba completions bash > /etc/bash_completion.d/ba
...
_ba_completions() {
    ...
    local script_dir ba_bin
    ba_bin="$(command -v ba 2>/dev/null || echo /usr/bin/ba)"
    # Follow symlinks to find the real scripts/ directory
    if [[ -L "$ba_bin" ]] && command -v readlink &>/dev/null; then
        script_dir="$(cd -- "$(dirname -- "$(readlink -f "$ba_bin")")" 2>/dev/null && pwd)"
    else
        script_dir="$(cd -- "$(dirname -- "$ba_bin")" 2>/dev/null && pwd)"
    fi
    local registry=""
    local candidate
    for candidate in \
        "${script_dir}/lib/ipc-registry.sh" \
        "${XDG_CONFIG_HOME:-$HOME/.config}/quickshell/ba/scripts/lib/ipc-registry.sh" \
        "/usr/share/quickshell/ba/scripts/lib/ipc-registry.sh" \
        "/usr/local/share/quickshell/ba/scripts/lib/ipc-registry.sh"; do
        if [[ -f "$candidate" ]]; then
            registry="$candidate"
            break
        fi
