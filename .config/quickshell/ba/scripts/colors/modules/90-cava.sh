...
MARKER_BEGIN="# BEGIN ba-managed"
MARKER_END="# END ba-managed"
...
apply_cava_config() {
  ...
  if grep -qF "$MARKER_BEGIN" "$CAVA_CONFIG"; then
    ...
    local legacy_begin="# BEGIN ba-generated-colors"
    if grep -qF "$legacy_begin" "$CAVA_CONFIG"; then
      local tmp; tmp=$(mktemp)
      awk -v begin="$legacy_begin" -v end="# END ba-generated-colors" -v block="$block" '
        $0 == begin { skip=1; printed=0 }
        skip && $0 == end { skip=0; print block; printed=1; next }
        !skip { print }
      ' "$CAVA_CONFIG" > "$tmp"
...
strip_cava_config() {
  ...
  for marker in "$MARKER_BEGIN" "# BEGIN ba-generated-colors"; do
  ...
  $stripped && log_module "Stripped BlueArchive config from cava"
}
