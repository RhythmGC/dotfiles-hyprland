...
VSCODE_THEMEGEN_BIN="$STATE_DIR/user/generated/bin/ba-vscode-themegen"
OPENCODE_THEMEGEN_BIN="$STATE_DIR/user/generated/bin/ba-opencode-themegen"
...
# Only deletes if the file was generated by BlueArchive (checks for known markers).
strip_neovim_spec() {
  [[ -f "$NEOVIM_THEME_FILE" ]] || return 0

  if grep -Fq 'BlueArchive generated theme.' "$NEOVIM_THEME_FILE" || grep -Fq '"bjarneo/aether.nvim"' "$NEOVIM_THEME_FILE" || grep -Fq 'name = "ba-neovim"' "$NEOVIM_THEME_FILE"; then
