...
blueArchiveConfigPath="$(ba_config_file)"
currentWallpaperPath=$(jq -r '.background.wallpaperPath' $blueArchiveConfigPath)
...
multiMonitorEnabled=$(jq -r '.background.multiMonitor.enable' "$blueArchiveConfigPath" 2>/dev/null)
