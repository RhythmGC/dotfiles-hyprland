...
def generate_kitty_config(colors, output_path):
    ...
    config = f"""# Auto-generated by ba wallpaper theming system
...
def fix_alacritty_import_order(config_path):
    ...
    new_lines = [
        "# BlueArchive wallpaper theming - import must be at top to override colors",
        "[general]",
        import_line,
    ]
    ...
    for line in lines:
        ...
        # Skip old bare import lines and their BlueArchive comments
        if re.match(bare_import_pat, stripped):
            continue
        if stripped.startswith("# BlueArchive wallpaper theming"):
            continue
        ...
        if re.match(r"^\[colors\.(primary|normal|bright|cursor|selection)\]", stripped):
            if not added_colors_comment:
                ...
                new_lines.append("")
                new_lines.append(
                    "# Color definitions commented out by BlueArchive wallpaper theming"
                )
                new_lines.append(
...
"""
    ...
    if ensure_line_in_file(ghostty_conf, "theme = ba-auto", r"theme\s*=\s*ba-auto"):
...
[General]
Description=ba Auto-generated Theme
Opacity=1.0
Wallpaper=
"""
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
...
"""
    ...
    if os.path.exists(starship_conf):
        ...
        # Check if palette_name is already set to ba
        if 'palette = "ba"' in content:
            print(f"✓ Generated Starship palette (already using ba palette)")
        else:
            # Add palette directive if not present
            if "palette =" not in content:
                # Add at top of file
                new_content = 'palette = "ba"\n\n' + content
                Path(starship_conf).write_text(new_content)
                content = new_content
                print(f"✓ Generated Starship palette and set as active")
            else:
                print(
                    f"✓ Generated Starship palette (using different palette, change to 'palette = \"ba\"' to use)"
                )

        # Update or append the [palettes.ba] section in starship.toml
        # Starship doesn't support source/include, so palette must be inline
        if "[palettes.ba]" in content:
            # Replace existing palette section with updated colors
            # Find start of [palettes.ba] and end (next section header or EOF)
            pattern = r"\[palettes\.ba\].*?(?=\n\[|\Z)"
            # Extract just the palette block from the generated config
            palette_block = config.strip().split("\n")
            # Skip comment lines at the top, keep from [palettes.ba] onward
            palette_start = next(
                i
                for i, line in enumerate(palette_block)
                if line.startswith("[palettes.ba]")
            )
            palette_content = "\n".join(palette_block[palette_start:])
            new_content = re.sub(pattern, palette_content, content, flags=re.DOTALL)
            if new_content != content:
                Path(starship_conf).write_text(new_content)
                print(f"  → Updated ba palette in starship.toml")
        else:
            with open(starship_conf, "a") as f:
                f.write("\n" + config)
            print(f"  → Appended ba palette to starship.toml")
    else:
        print(
            f"✓ Generated Starship palette (starship.toml not found - create it and add 'palette = \"ba\"')"
        )
    ...
    if btop_path.exists():
        ...
        if re.search(r"^color_theme\s*=", content, re.MULTILINE):
            ...
            if new_content != content:
                ...
                print(f"\u2713 Generated btop theme (already using ba-auto)")
        else:
        ...
        btop_path.write_text(f'color_theme = "ba-auto"\n')
...
"""
    ...
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    with open(output_path, "w") as f:
        f.write(f"# Auto-generated by ba wallpaper theming system\n")
...
def generate_yazi_config(colors, output_path):
    """Generate yazi flavor for ba theming.

    Creates a flavor directory at ~/.config/yazi/flavors/ba-auto.yazi/
...
"""
    ...
    flavor_line = 'use = "ba-auto"'
    if theme_path.exists():
        content = theme_path.read_text()
        if "[flavor]" in content:
            if re.search(r'^use\s*=\s*"ba-auto"', content, re.MULTILINE):
                print(f"\u2713 Generated yazi flavor (already using ba-auto)")
...
def main():
    ...
    if "ghostty" in terminals:
        generate_ghostty_config(colors, f"{home}/.config/ghostty/themes/ba-auto")

    if "konsole" in terminals:
        generate_konsole_config(
            colors, f"{home}/.local/share/konsole/ba-auto.colorscheme"
        )

    if "starship" in terminals:
        generate_starship_config(colors, f"{home}/.config/starship/ba-palette.toml")
    ...
    if "omp" in terminals:
        ...
        try:
            with open(colors_json_path, "r") as f:
                ...
                generate_omp_config(
                    omp_colors, f"{home}/.config/oh-my-posh/ba-auto.json"
                )
        except FileNotFoundError:
            print(
                f"Warning: {colors_json_path} not found, oh-my-posh theme may be incomplete"
            )
            generate_omp_config(colors, f"{home}/.config/oh-my-posh/ba-auto.json")
        ...
        try:
            with open(colors_json_path, "r") as f:
                ...
                generate_btop_config(
                    btop_colors, f"{home}/.config/btop/themes/ba-auto.theme"
                )
        except FileNotFoundError:
            print(
                f"Warning: {colors_json_path} not found, btop theme may be incomplete"
            )
            generate_btop_config(colors, f"{home}/.config/btop/themes/ba-auto.theme")

    if "lazygit" in terminals:
        generate_lazygit_config(colors, f"{home}/.config/lazygit/ba-theme.yml")

    if "yazi" in terminals:
        generate_yazi_config(
            colors, f"{home}/.config/yazi/flavors/ba-auto.yazi/flavor.toml"
        )
    ...
    if args.zed:
        generate_zed_config(
            colors,
            args.scss,
            f"{home}/.config/zed/themes/ba-theme.json",
            args.colors,
            args.terminal_json,
        )
