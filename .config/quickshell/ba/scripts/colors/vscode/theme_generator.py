...
def generate_vscode_colors(colors, scss_path):
    """Generate VSCode workbench.colorCustomizations from BlueArchive theme data."""
...
def _write_extension_manifest(ext_dir: Path) -> None:
    ...
    manifest = {
        ...
        "description": "Dynamic Material You theme — auto-generated by BlueArchive from wallpaper colors",
        "version": "1.0.0",
        "publisher": "ba",
...
LEGACY_EXTENSION_DIRS = ["ba.ba-theme-1.0.0"]


def _clean_legacy_extensions(fork_key: str) -> None:
    """Remove old extension directories from previous BlueArchive versions."""
    import shutil
...
def _ensure_extension_registered(fork_key: str, ext_dir: Path) -> None:
    ...
    ext_id = f"ba.{THEME_EXTENSION_ID}"
    for e in entries:
...
def generate_vscode_theme(colors_json_path, scss_path, settings_path, fork_key=""):
    ...
    # Clean up legacy extensions from older BlueArchive versions
    ...
    # Remove legacy injection keys from previous BlueArchive versions
    for key in [
...
THEME_NAME = "BlueArchive Material"
THEME_EXTENSION_ID = "ba-material-theme"
THEME_FILE_NAME = "ba-material-color-theme.json"
PREV_THEME_KEY = "ba.previousColorTheme"
...
def strip_vscode_theme(settings_path: str, fork_key: str = "") -> bool:
    """Remove BlueArchive theme extension and settings.json entries."""
    import shutil
    ...
    if changed:
        try:
            with open(settings_path, "w") as f:
                json.dump(settings, f, indent=2, ensure_ascii=False)
            print(f"✓ Stripped BlueArchive theme from {settings_path}")
        except OSError as e:
...
def strip_all_vscode_themes(forks: list = None):
    """Strip BlueArchive theme from multiple VSCode forks."""
    if forks is None:
        forks = [
            key
...
def generate_all_vscode_themes(
    ...
    Args:
        colors_json_path: Path to BlueArchive palette.json (colors.json fallback is still accepted)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--strip",
        action="store_true",
        help="Remove BlueArchive color customizations from settings.json (used when disabling theming)",
    )
