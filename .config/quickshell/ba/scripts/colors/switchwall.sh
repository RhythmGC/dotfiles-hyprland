...
SHELL_CONFIG_FILE="$(ba_config_file)"
 ...
 resolve_effective_theming_wallpaper() {
    jq -r --arg focused_monitor "$(get_focused_monitor_name)" '
        ...
        if (.appearance.wallpaperTheming.useBackdropForColors // false) then
            ...
            if (.panelFamily // "ba") == "waffle" then waffle_main else main_path end
        end // ""
...
switch() {
    ...
    # Respects active panel family: ba reads from background.backdrop, waffle from waffles.background.backdrop
    if [[ "$color_flag" != "1" ]]; then
        if [[ "$cfg_use_backdrop" == "true" ]]; then
            ...
            if [[ "$cfg_panel_family" == "waffle" ]]; then
                ...
                backdrop_use_main="$cfg_ba_backdrop_use_main"
                backdrop_path="$cfg_ba_backdrop_path"
            fi
    ...
    _ba_python="$_ba_venv/bin/python3"
    [[ ! -x "$_ba_python" ]] && _ba_python="python3"
    ...
    if "$_ba_python" "$SCRIPT_DIR/generate_colors_material.py" "${generate_colors_material_args[@]}" \
    ...
    if [[ "$force_dark_terminal" == "true" ]]; then
        ...
        scss_cmd=("$_ba_python" "$SCRIPT_DIR/generate_colors_material.py" "${scss_generate_args[@]}")
...
main() {
    ...
    cfg_panel_family="${_cfg[6]:-ba}"
    cfg_waffle_backdrop_use_main="${_cfg[7]:-true}"
    cfg_waffle_backdrop_path="${_cfg[8]:-}"
    cfg_ba_backdrop_use_main="${_cfg[9]:-true}"
    cfg_ba_backdrop_path="${_cfg[10]:-}"
