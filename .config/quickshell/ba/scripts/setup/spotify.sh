...
#               in config.json — applies the BlueArchive Spicetify theme.
...
CONFIG_PATH="${XDG_CONFIG_HOME:-$HOME/.config}/baOS/config.json"
...
if is_arch_like; then
    ...
    if _theme_enabled_in_config; then
        setup_progress 5 $TOTAL "Applying BlueArchive Spicetify theme"
        theme_script="$SCRIPT_DIR/../colors/apply-spicetify-theme.sh"
        if [[ -x "$theme_script" ]]; then
            if "$theme_script"; then
                echo "BlueArchive theme applied."
            else
        ...
        setup_progress 5 $TOTAL "Skipping BlueArchive theme (appearance.wallpaperTheming.enableSpicetify is off)"
        echo "  · Enable it in Settings → Themes → 'Spotify theming' to apply the BlueArchive theme."
    fi
