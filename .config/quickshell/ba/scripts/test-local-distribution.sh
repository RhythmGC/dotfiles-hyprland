...
launcher="${BA_LAUNCHER_PATH:-$runtime_root/scripts/ba}"
...
if [[ -f "$runtime_root/Makefile" ]]; then
    ...
    make -n install PREFIX=/tmp/ba-stage-test -C "$runtime_root" >/dev/null
fi

if [[ -d "$runtime_root/distro/arch" ]]; then
    step "pkgbuild syntax"
    bash -n \
        "$runtime_root/distro/arch/ba-shell/PKGBUILD" \
        "$runtime_root/distro/arch/ba-shell-git/PKGBUILD" \
        "$runtime_root/distro/arch/ba-meta/PKGBUILD"

    step "version consistency"
    version="$(cat "$runtime_root/VERSION")"
    for pkg in ba-shell ba-meta; do
...
if [[ "$run_runtime" == true ]]; then
    ...
    bash scripts/ba kill >/dev/null 2>&1 || true
    sleep 1
    bash scripts/ba run >/tmp/ba-test-local-runtime.log 2>&1 &
    sleep 3

    step "runtime logs"
    bash scripts/ba logs
