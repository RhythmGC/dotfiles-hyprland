...
def _find_ba_pid():
    """Locate the PID of the running BlueArchive quickshell process by inspecting /proc.
...
def get_ba_env():
    """Get relevant environment variables from the running BlueArchive quickshell
    ...
    """
    global BA_ENV_CACHE, BA_ENV_PID
    try:
        pid = _find_ba_pid()
        if pid is None:
        ...
        if BA_ENV_PID == pid and BA_ENV_CACHE:
            return BA_ENV_CACHE
