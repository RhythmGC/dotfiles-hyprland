#!/usr/bin/env bash
# Install ba-pixel SDDM theme for BlueArchive
...
# Old name (legacy, pre-2.26): /etc/sddm.conf.d/ba-theme.conf — cleaned up below.
SDDM_CONF="/etc/sddm.conf.d/99-ba-theme.conf"
SDDM_CONF_LEGACY="/etc/sddm.conf.d/ba-theme.conf"
AUTO_APPLY_MODE="${BA_SDDM_AUTO_APPLY:-ask}" # ask|yes|no
...
should_apply_theme() {
    ...
    if [[ "$AUTO_APPLY_MODE" == "no" ]]; then
        log_info "Skipping SDDM Current theme switch by policy (BA_SDDM_AUTO_APPLY=no)"
        return 1
    fi
...
# This allows IPC-triggered updates (ba shell update) to refresh the theme
...
if [[ ! -f "${THEME_DIR}/assets/background.png" ]]; then
    ...
    # Copy default wallpaper from BlueArchive assets as initial background
...
# Migrate from the old SDDM_CONF filename (ba-theme.conf) to the new
# alphabetical-last name (99-ba-theme.conf). Done idempotently: if the legacy
...
# On updates where ba-pixel is already active, we still need to patch settings.
...
if [[ "$current_conf" == "$desired_conf" ]]; then
    ...
    if should_apply_theme; then
        ...
        # Theme is already ba-pixel but settings are stale — update without asking
...
if [[ -f "$MATUGEN_CONFIG" ]]; then
    if grep -qE "post_hook\s*=\s*'.*sudo.*sync-pixel-sddm\.py" "$MATUGEN_CONFIG" 2>/dev/null; then
        ...
        log_warn "Please remove old ba-pixel-sddm hook block from: $MATUGEN_CONFIG"
    fi
fi
...
log_info "Colors auto-sync on wallpaper change via the BlueArchive theming pipeline"
