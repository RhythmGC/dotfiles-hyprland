#!/usr/bin/env python3
"""Sync ba-pixel SDDM theme colors with current Material You palette.

Reads generated colors from BlueArchive's generated palette and updates
the ba-pixel theme.conf with matching colors.
Reads wallpaper path from BlueArchive state and updates background image.
...
[SddmTheme]
Name=ba-pixel
Description=BlueArchive SDDM login screen — Material You dynamic colors
Type=sddm-theme
Author=BlueArchive project
Version=1.0
Website=https://github.com/RhythmGC/dotfiles-hyprland
Screenshot=
MainScript=Main.qml
...
# BlueArchive Material You colors — updated automatically by sync-pixel-sddm.py on wallpaper change"""
...
def read_colors():
    """Read Material You colors from BlueArchive's generated palette.
...
def read_wallpaper():
    """Read current wallpaper path from BlueArchive config.json."""
    ...
    try:
        ...
        panel_family = config.get("panelFamily", "ba")
...
def update_theme_conf(colors):
    """Update ba-pixel theme.conf [General] section with new colors.
