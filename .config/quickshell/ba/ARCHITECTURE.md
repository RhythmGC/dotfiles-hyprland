...
Originally forked from [end-4/dots-hyprland](https://github.com/end-4/dots-hyprland) (baOS). Secondary Hyprland support is maintained.
...
| Styles | material, cards, aurora, ba, angel | Single fluent style |
...
| Panels | 24 (baBar, baDock, baSidebarLeft, ...) | 24 (wBar, wStartMenu, wActionCenter, ... + shared ba panels) |
...
Style dispatch priority: **angel > ba > aurora > material**. Cards is a material variant (no separate dispatch).

## Directory Structure

```
shell.qml                     # Root entry — loads services, selects panel family
ShellBaPanels.qml             # Material Design family (24 panels)
...
│   ├── Appearance.qml        # ba visual tokens (881 lines, 400+ properties)
│   ├── Config.qml            # Central config (JsonAdapter, 1385+ lines)
│   └── widgets/              # 129 reusable widgets + qmldir
├── bar/                      # Top bar (ba family, 33 files)
...
├── ba                      # CLI launcher (30KB bash, 40+ commands)
...
| User file | `~/.config/baOS/config.json` (legacy namespace from fork origin) |
...
| `Appearance` | 352+ | All ba module visuals |
...
Called externally: `ba <target> <function> [args]`
...
- **Repo-sync**: `./setup install` → syncs to `~/.config/quickshell/ba/`
- **Package-managed**: `make install` → copies to `/usr/share/quickshell/ba/`

User config for the running QML shell lives at `~/.config/baOS/config.json` (legacy namespace, persistent across updates). NOTE: the shell scripts/CLI default to `~/.config/baOS/` with a legacy fallback — the two sides are not yet unified.
...
ba restart                # Graceful restart
ba logs | tail -50        # Check for errors
ba status                 # Runtime health check
ba doctor                 # Auto-diagnose + fix
ba settings               # Open settings GUI

# IPC calls
ba <target> <function> [args...]
ba overview toggle
ba audio volumeUp

# Low-level restart (fallback)
qs kill -c ba; qs -c ba
```
