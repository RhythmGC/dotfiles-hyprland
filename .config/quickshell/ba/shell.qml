...
ShellRoot {
    ...
    readonly property bool disableHotReload: Quickshell.env("BA_DISABLE_HOT_RELOAD") === "1"
        || Quickshell.env("BA_DISABLE_HOT_RELOAD") === "true"
    ...
    // Boot phase timing (ms since epoch). Written to ~/.cache/ba/last-boot.json
    // when the deferred phase finishes. `ba status` reads this back to show users
    ...
    readonly property string _bootCachePath: (Quickshell.env("XDG_CACHE_HOME") || (Quickshell.env("HOME") + "/.cache")) + "/ba/last-boot.json"
    ...
    // Persist boot phase timestamps so `ba status` can report startup breakdown
    ...
    Connections {
        ...
        function onReadyChanged() {
            if (Config.ready) {
                ...
                // Migration: Ensure waffle family has wBackdrop instead of baBackdrop
    ...
    function migrateEnabledPanels() {
        ...
        if (family === "waffle") {
            // If waffle family has baBackdrop but not wBackdrop, migrate
            const hasBaBackdrop = panels.includes("baBackdrop");
            const hasWBackdrop = panels.includes("wBackdrop");

            if (hasBaBackdrop && !hasWBackdrop) {
                root._log("[Shell] Migrating enabledPanels: replacing baBackdrop with wBackdrop for waffle family");
                panels = panels.filter(p => p !== "baBackdrop");
    ...
    // This saves ~135 file parses when using ba family (waffle not parsed) and vice versa.
    LazyLoader {
        active: Config.ready && (Config.options?.panelFamily ?? "ba") !== "waffle"
        source: "ShellBaPanels.qml"
    }
    ...
    // === Panel Families ===
    // Note: baAltSwitcher is always loaded (not in families) as it acts as IPC router
    // for the unified "altSwitcher" target, redirecting to wAltSwitcher when waffle is active
    property list<string> families: ["ba", "waffle"]
    property var panelFamilies: ({
        "ba": [
            "baBar", "baBackground", "baBackdrop", "baBootGreeting", "baIdleOverlay", "baCheatsheet", "baControlPanel", "baDock", "baLock",
            ...
            "baWallpaperSelector", "baCoverflowSelector", "baClipboard", "baShellUpdate", "baRecordingOsd", "baScreenTranslator"
        ],
        "waffle": [
            ...
            "baBootGreeting", "baIdleOverlay", "baCheatsheet", "baOnScreenKeyboard", "baOverlay", "baOverview",
            "baRegionSelector", "baScreenCorners", "baWallpaperSelector", "baCoverflowSelector", "baClipboard", "baScreenTranslator"
        ]
    })
    ...
    function cyclePanelFamily() {
        const currentFamily = Config.options?.panelFamily ?? "ba"
        const currentIndex = families.indexOf(currentFamily)
        const nextIndex = (currentIndex + 1) % families.length
        const nextFamily = families[nextIndex]

        // Determine direction: ba -> waffle = left, waffle -> ba = right
