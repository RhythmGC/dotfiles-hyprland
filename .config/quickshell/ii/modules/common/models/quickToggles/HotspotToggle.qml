 ...
 *
 * Config keys read:
 *   hotspot.ssid     — broadcast network name (default: "ba Hotspot")
 *   hotspot.password — WPA2 passphrase        (default: "bahotspot")
...
QuickToggleModel {
    ...
    statusText: root.toggled
        ? (Config.options?.hotspot?.ssid ?? "ba Hotspot")
        : Translation.tr("Off")
    ...
    mainAction: () => {
        if (root.toggled) {
            ...
            const ssid = Config.options?.hotspot?.ssid ?? "ba Hotspot"
            const password = Config.options?.hotspot?.password ?? "bahotspot"
