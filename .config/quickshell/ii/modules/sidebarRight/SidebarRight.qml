...
Scope {
    ...
    PanelWindow {
        ...
        mask: Region {
            item: GlobalStates.sidebarRightOpen ? maskItem : null
        }
        ...
        Loader {
            ...
            Keys.onPressed: (event) => {
                ...
                const isInput = activeFocusItem && (
                    activeFocusItem.hasOwnProperty("echoMode") || 
                    activeFocusItem.hasOwnProperty("readOnly") || 
                    activeFocusItem.hasOwnProperty("selectByMouse")
                );

                if (!isInput) {
                    if (event.key === Qt.Key_Up) {
                        if (sidebarContentLoader.item && sidebarContentLoader.item.decrementBottomTab) {
                            sidebarContentLoader.item.decrementBottomTab();
                            event.accepted = true;
                        }
                    }
                    else if (event.key === Qt.Key_Down) {
                        if (sidebarContentLoader.item && sidebarContentLoader.item.incrementBottomTab) {
                            sidebarContentLoader.item.incrementBottomTab();
                            event.accepted = true;
                        }
                    }
                }
