...
Item {
    ...
    readonly property var defaultOrder: ["clock", "notes", "media", "wallpaper", "context", "launch", "status", "crypto"]
    readonly property int widgetSpacing: Config.options?.sidebar?.widgets?.spacing ?? 8

    readonly property bool showClock: Config.options?.sidebar?.widgets?.clock ?? true
    readonly property bool showNotes: Config.options?.sidebar?.widgets?.notes ?? true
    ...
    readonly property bool showWallpaper: Config.options?.sidebar?.widgets?.wallpaper ?? true

    readonly property var visibleWidgets: {
        const order = widgetOrder ?? defaultOrder
        return order.filter(id => {
            switch (id) {
            case "clock": return showClock
