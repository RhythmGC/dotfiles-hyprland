...
Item {
    ...
    readonly property var wallpapersPaths: [
        `${FileUtils.trimFileProtocol(Directories.pictures)}/Wallpapers`,
        `${FileUtils.trimFileProtocol(Directories.pictures)}/homework`
    ]
    ...
    Process {
        ...
        command: ["/usr/bin/fish", "-c", `find ${root.wallpapersPaths.map(path => `'${path}'`).join(" ")} -maxdepth 1 -type f \\( -iname '*.jpg' -o -iname '*.jpeg' -o -iname '*.png' -o -iname '*.webp' -o -iname '*.avif' -o -iname '*.bmp' -o -iname '*.svg' \\) -printf '%C@\\t%p\\n' 2>/dev/null`]
        stdout: SplitParser {
            splitMarker: ""
            onRead: data => {
